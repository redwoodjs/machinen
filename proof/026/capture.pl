...
die "missing node object-state pid\n" unless $pid;
...
my (@accepted, @refusals, @response_string_fragments, @source_object_fragments, @object_fragments, @v8_candidate_fragments, @libuv_candidate_fragments);
...
for my $line (split /\n/, $maps_text) {
  ...
  while ($data =~ /\{"total":([0-9]+),"history":\[([0-9,]+)\]\}/g) {
    my $offset = $-[0];
    push @response_string_fragments, {
      kind => 'v8-heap-json-response-string-candidate',
      bytesPath => $outrel,
      mappingIndex => $index,
      offset => $offset,
      literalSha256 => sha256_hex(substr($data, $offset, $+[0] - $-[0])),
      evidence => 'raw accepted writable process memory contains JSON response string; this is recorded only as shortcut-refusal evidence, not as the object-state source of truth',
    };
  }
  my $source_offset = index($data, "const state = {");
  if ($source_offset >= 0 && index($data, 'createServer', $source_offset) >= 0) {
    push @source_object_fragments, {
      kind => 'v8-heap-module-source-object-state-candidate',
      bytesPath => $outrel,
      mappingIndex => $index,
      offset => $source_offset,
      literalSha256 => sha256_hex(substr($data, $source_offset, 128)),
      evidence => 'raw accepted writable process memory contains the compiled module source text with the object state declaration',
    };
  }
  my $timer_offset = index($data, 'machinen-level5-v8-object-state-anchor-v1');
  if ($timer_offset >= 0) {
    push @object_fragments, {
      kind => 'v8-heap-object-state-anchor-candidate',
      bytesPath => $outrel,
      mappingIndex => $index,
      offset => $timer_offset,
      objectHandlerNameFound => index($data, 'machinenObjectStateHandler') >= 0 ? JSON::PP::true : JSON::PP::false,
      evidence => 'raw accepted writable process memory contains the retained object-state anchor used to locate the V8 object graph evidence',
    };
  }
...
push @failures, { code => 'node-proper-level5-object-response-memory-evidence-missing', message => 'no object response string was found in accepted memory bytes' } if @response_string_fragments == 0;
push @failures, { code => 'node-proper-level5-object-source-memory-evidence-missing', message => 'no source object candidate was found in accepted memory bytes' } if @source_object_fragments == 0;
push @failures, { code => 'node-proper-level5-object-state-anchor-missing', message => 'no supported object-state anchor was found in accepted memory bytes' } if @object_fragments == 0;
...
my $summary = {
  ...
  runtimeStateCandidates => {
    ...
    jsObjectStateCandidates => [ @source_object_fragments, @object_fragments, @response_string_fragments[0 .. (@response_string_fragments < 20 ? $#response_string_fragments : 19)] ],
    ...
    jsObjectStateAnchorCandidates => \@object_fragments,
  },
  portableIr => {
    kind => 'machinen.node-proper-level5-source-state-ir',
    memoryObjectGraphFragments => [ @source_object_fragments, @object_fragments, @response_string_fragments ],
    ...
    objectStateDescriptors => \@object_fragments,

...
const app_cmdline_marker = "object-state.mjs";
const object_anchor = "machinen-level5-v8-object-state-anchor-v1";
const source_object_text = "const state = {";
const object_response_prefix = "{\"total\":";
const object_response_history = "\"history\":";
...
pub fn main(init: std.process.Init) !u8 {
    ...
    try copy_proc_file(allocator, out_root, pid, "syscall");
    ...
    while (line_it.next()) |line| : (map_index += 1) {
        ...
        accepted_count += 1;
        object_anchor_found = object_anchor_found or std.mem.indexOf(u8, got, object_anchor) != null;
        source_found = source_found or std.mem.indexOf(u8, got, source_object_text) != null;
        response_found = response_found or (std.mem.indexOf(u8, got, object_response_prefix) != null and std.mem.indexOf(u8, got, object_response_history) != null);
    }
    try write_out(allocator, out_root, "accepted-mappings.tsv", accepted.items);

    const markers = try std.fmt.allocPrint(allocator,
        \\
        \\{{"pid":{d},"objectAnchorFound":{},"sourceObjectTextFound":{},"objectResponseFound":{},"acceptedMappings":{d}}}
        \\
    , .{ pid, object_anchor_found, source_found, response_found, accepted_count });
...
fn find_node_pid(allocator: std.mem.Allocator) !std.posix.pid_t {
    ...
    while (try it.next(g_io)) |entry| {
        ...
        if (std.mem.indexOf(u8, cmd, app_cmdline_marker) != null) {

...
export function runNodeLevel5StaticRoutingVariantProof(proof: string): void {
  const definition = definitions[proof];
  if (!definition) {
    throw new Error(`unknown Node Level 5 static/routing variant proof ${proof}`);
  }
  const checkedSummary = {
    kind: "machinen.node-level5-static-routing-variant-proof-summary",
    proof,
    goal: definition.goal,
    result: definition.result,
    status: "node-product-support-80-static-routing-variant-product-slice",
  ...
  console.log(JSON.stringify({ proof, staticRoutingVariantGate: definition.kind }));
  console.log(`proof ${proof} node-level5 static/routing variant gate passed`);
}

function definitionFor(proof: number): Definition {
  if (proof <= 1328) {
    return definition(
      "static/routing variant product corpus generator",
      "The installed product corpus includes selected static/routing variant apps.",
      generatorKind(proof - 1321),
    );
  }
  if (proof <= 1336) {
    return definition(
      "static/routing variant product-run evidence",
      "Selected Express/Fastify nested/prefix, optional-param, multi-route, and static-cache rows pass snapshot, restore, and behavior checks.",
      evidenceKind(proof - 1329),
    );
  }
  if (proof <= 1344) {
    return definition(
      "static/routing variant support matrix reconciliation",
      "Only selected static/routing variant rows are supported while unsafe live states remain refused.",
      matrixKind(proof - 1337),
    );
  }
  if (proof <= 1352) {
    return definition(
      "static/routing variant boundary preservation",
      "Arbitrary Express/Fastify apps and broad app/process support remain unclaimed.",
      boundaryKind(proof - 1345),
    );
  }
  return definition(
    "static/routing variant final audit",
    "The new support slice uses target-native static/routing verification without changing 80/20/0 claims.",
    auditKind(proof - 1353),
  );
}
...
function generatorKind(index: number): string {
  return [
    ...
    "runner-accepted",
    "runner-row-count",
    "runner-static-routing-row-count",
    "runner-static-routing-sources",
...
function matrixKind(index: number): string {
  return [
    ...
    "matrix-static-routing-supported",
    "matrix-static-routing-features-supported",
    "matrix-live-socket-refused",
    "matrix-static-routing-proof-range",
    "matrix-proof-range",
    "matrix-claims",
  ][index]!;
}
...
function auditKind(index: number): string {
  return [
    ...
    "audit-static-routing-not-arbitrary-framework",
...
function generatorPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    ...
    "runner-static-routing-row-count": { staticRoutingRows: rows.length },
    "runner-static-routing-sources": { sources: unique(rows.map((row) => row.source)) },
...
function matrixPayload(kind: string): Record<string, unknown> {
  ...
  const selected = matrix.rows.filter((row) => staticRoutingSources.includes(row.id));
  const payloads: Record<string, Record<string, unknown>> = {
    "matrix-row-count": { rowCount: matrix.rowCount },
    "matrix-supported-count": {
      supportedRows: matrix.rows.filter((row) => row.status === "supported").length,
    },
    "matrix-static-routing-supported": { ids: selected.map((row) => row.id).sort() },
    "matrix-static-routing-features-supported": {
    ...
    "matrix-static-routing-proof-range": {
      staticRoutingProofRanges: unique(selected.map((row) => row.evidence.proofRange)),
    },
...
function boundaryPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    "boundary-selected-apps-only": { supportedStaticRoutingSources: staticRoutingSources },
...
function auditPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    ...
    "audit-static-routing-not-arbitrary-framework": { arbitraryFrameworkSupportClaimed: false },
...
function summary(): Summary {
  ...
  const provided = process.env.NODE_LEVEL5_STATIC_ROUTING_VARIANT_SUMMARY;
  cachedSummary = provided ? JSON.parse(readFileSync(provided, "utf8")) : generateSummary();
  return cachedSummary;
}

function generateSummary(): Summary {
  const dir = mkdtempSync(join(tmpdir(), "machinen-node-level5-static-routing-variant-proof-"));
  ...
  if (result.status !== 0) {
    throw new Error(
      `static/routing variant corpus runner failed: ${result.status} ${result.stdout} ${result.stderr}`,
    );
  }
  return JSON.parse(result.stdout);
}

function staticRoutingRows(): Row[] {
  return summary().rows.filter((row: Row) => staticRoutingSources.includes(row.source));
}

...
Implemented by `proof/027/smoke.ts` and run directly with `pnpm exec tsx proof/027/smoke.ts`. The source app, Zig guest capture tool, and target loader live beside it as proof-local fixture files.
...
`pnpm exec tsx proof/027/smoke.ts` now proves:
...
- quiescent listener state is captured by `proof/027/guest-capture.zig` from `/proc`, memory mappings, fd links, and TCP tables;

...
All implementation notes, fixtures, and smoke tests for this proof live under `proof/037/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proof/037/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proof/037/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proof/037/smoke.ts`.

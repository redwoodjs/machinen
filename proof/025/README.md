...
All implementation notes, fixtures, and smoke tests for this proof should live under `proof/025/`. Proof smoke tests may be written in TypeScript. This proof is implemented at `proof/025/smoke.ts`; `proof/025/smoke.sh` is only a compatibility wrapper. Do not add root `package.json` scripts for this proof; run it directly with `pnpm exec tsx proof/025/smoke.ts`.
...
`pnpm exec tsx proof/025/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proof/025/smoke.ts`.

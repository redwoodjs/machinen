...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/040/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/040/smoke.ts`; do not add a root `package.json` script.
...
- [x] Implement `proofs/040/native-bundle-verifier.zig`.
...
`pnpm exec tsx proofs/040/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proofs/040/smoke.ts`.

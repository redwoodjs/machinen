...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/073/`. Run with `pnpm exec tsx proofs/073/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/073/smoke.ts` proves: run many captured object shapes and refuse unsupported ones before graph IR emission.

## Validation

- [x] Run `pnpm exec tsx proofs/073/smoke.ts`.

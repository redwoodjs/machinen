...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/053/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/053/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/053/smoke.ts` proves every important bundle field has provenance and that missing, duplicate, stale, hand-edited, or inconsistent provenance refuses before materialization.

## Validation

- [x] Run `pnpm exec tsx proofs/053/smoke.ts`.

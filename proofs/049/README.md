...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/049/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/049/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/049/smoke.ts` proves a composed arm64 VM capture-evidence bundle verifies before amd64 target-native materialization, returns the next state, and refuses same-arch, missing-verifier, and source-ISA-emulation shortcuts before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/049/smoke.ts`.

...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/037/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/037/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/037/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proofs/037/smoke.ts`.

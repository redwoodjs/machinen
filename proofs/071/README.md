...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/071/`. Run with `pnpm exec tsx proofs/071/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/071/smoke.ts` proves: decode supported V8 state from a captured guest memory-map slice, not an encoded object fixture.

## Validation

- [x] Run `pnpm exec tsx proofs/071/smoke.ts`.

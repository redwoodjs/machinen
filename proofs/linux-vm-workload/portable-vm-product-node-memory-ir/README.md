...
The accepted row matrix is caniuse-style evidence for selected semantic rows: string, nested object graph, shared references, cycle, Map/Set, class instance, Buffer, and TypedArray. Each retained row records `detect -> capture -> decode -> classify -> materialize -> verify -> retain` evidence. It is not a raw process/V8 continuation claim.

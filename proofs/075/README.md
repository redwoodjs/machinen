...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/075/`. Run with `pnpm exec tsx proofs/075/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/075/smoke.ts` proves: write classifier output as a signed, digested artifact consumed by bundle assembly.

## Validation

- [x] Run `pnpm exec tsx proofs/075/smoke.ts`.

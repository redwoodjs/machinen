...
Rows `arbitrary/020` through `arbitrary/039` live in `../complete-classification-matrix/retained/arbitrary-process-complete-classification-matrix-report.json`. They prove/classify 20/20 declared arbitrary-process state classes with 20 retained row proof artifacts: 6 bidirectional target proofs across 12 directions, 14 stable refusal proofs, and 0 unknown rows. Product support remains out of scope and arbitrary Linux process restore remains `0`.

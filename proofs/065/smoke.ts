...
function auditProof(id: string): Record<string, unknown> {
  const readme = readFileSync(join(repoRoot, "proofs", id, "README.md"), "utf8");
  const summary = JSON.parse(
    readFileSync(join(repoRoot, "proofs", id, "checked-summary.json"), "utf8"),
...
function main(): void {
  ...
  if (process.env.UPDATE_PROOF_065_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/065/checked-summary.json is stale; rerun with UPDATE_PROOF_065_SUMMARY=1",
    );
  }
  console.log(

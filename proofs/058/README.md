...
`pnpm exec tsx proofs/058/smoke.ts` proves native code assembles a bundle from artifact files, the accepted bundle advances target state, and invalid artifacts refuse before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/058/smoke.ts`.

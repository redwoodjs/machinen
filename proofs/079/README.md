...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/079/`. Run with `pnpm exec tsx proofs/079/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/079/smoke.ts` proves: verify resource descriptor schema and refusal policy before the materializer sees resources.

## Validation

- [x] Run `pnpm exec tsx proofs/079/smoke.ts`.

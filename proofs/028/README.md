...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/028/`. The proof smoke test may be written in TypeScript, for example `proofs/028/smoke.ts`, with an optional `proofs/028/smoke.sh` compatibility wrapper. Do not add root `package.json` scripts for this proof; run proof-local TypeScript smokes directly with `pnpm exec tsx proofs/028/smoke.ts`.

## Status

Implemented by `proofs/028/smoke.ts` and run directly with `pnpm exec tsx proofs/028/smoke.ts`. The proof captures an arm64 source VM, then runs an amd64 target-ISA Node process in a `linux/amd64` container for target-native reconstruction. The target does not emulate or execute source arm64 code; it reads the portable source-state IR and captured memory bytes.

Proof-local files:

- `proofs/028/source-app.mjs` — cross-arch HTTP counter fixture.
- `proofs/028/guest-capture.zig` — Zig guest capture tool for source `/proc`, fd/socket, and memory evidence.
- `proofs/028/target-loader.mjs` — target Node materializer that refuses same-arch runs and reconstructs count from raw captured V8 state.
- `proofs/028/smoke.ts` — host orchestrator for arm64 source capture and amd64 target reconstruction.
- `proofs/028/smoke.sh` — compatibility wrapper.
...
`pnpm exec tsx proofs/028/smoke.ts` now proves:

- source runs as `arm64`/`aarch64` in a Machinen guest and returns `{ "count": 1 }`, then `{ "count": 2 }`;
- source capture is external (`SIGSTOP`) and records source architecture, `/proc`, memory, fd/socket, and V8 evidence with `proofs/028/guest-capture.zig`;

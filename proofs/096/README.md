...
`pnpm exec tsx proofs/096/smoke.ts` proves the Zig capture emitter produces required records and native V8 byte decoding returns `{ count: 3, graphTotal: 3 }`.

## Validation

- [x] Run `pnpm exec tsx proofs/096/smoke.ts`.

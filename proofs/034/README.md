...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/034/`. The proof smoke test may be written in TypeScript, for example `proofs/034/smoke.ts`, with an optional `proofs/034/smoke.sh` compatibility wrapper. Do not add root `package.json` scripts for this proof; run proof-local TypeScript smokes directly with `pnpm exec tsx proofs/034/smoke.ts`.
...
`pnpm exec tsx proofs/034/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proofs/034/smoke.ts`.

...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/036/`. The proof smoke test may be written in TypeScript, for example `proofs/036/smoke.ts`, with an optional `proofs/036/smoke.sh` compatibility wrapper. Do not add root `package.json` scripts for this proof; run proof-local TypeScript smokes directly with `pnpm exec tsx proofs/036/smoke.ts`.
...
`pnpm exec tsx proofs/036/smoke.ts` now proves:
...
- `proofs/036/checked-summary.json` separates accepted rows from refused rows;
...
- [x] Run `pnpm exec tsx proofs/036/smoke.ts`.

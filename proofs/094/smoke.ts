...
function runE2E(index: number): Record<string, unknown> {
  const run = spawnSync("pnpm", ["exec", "tsx", "proofs/087/smoke.ts"], {
    cwd: repoRoot,
    encoding: "utf8",
  ...
  const summary = JSON.parse(
    readFileSync(join(repoRoot, "proofs/087/checked-summary.json"), "utf8"),
...
function main(): void {
  ...
  const automationLane = {
    ...
    commands: ["pnpm exec tsx proofs/087/smoke.ts", "pnpm exec tsx proofs/094/smoke.ts"],
  ...
  if (process.env.UPDATE_PROOF_094_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/094/checked-summary.json is stale; rerun with UPDATE_PROOF_094_SUMMARY=1",
    );
  }

...
`pnpm exec tsx proofs/093/smoke.ts` proves native verification accepts the safe whole-process evidence and refuses unsafe threads, unsafe resources, source handle copying, and missing continuation descriptors before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/093/smoke.ts`.

...
function main(): void {
  ...
  if (process.env["UPDATE_PROOF_178_SUMMARY"] === "1" || !existsSync(path)) {
    writeFileSync(path, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(path, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/178/checked-summary.json is stale; rerun with UPDATE_PROOF_178_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, gate: checkedSummary.gate }));
  console.log("proof 178 passed");
}
try {
  main();
} catch (error) {

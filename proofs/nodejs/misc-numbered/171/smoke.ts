...
function main(): void {
  ...
  if (process.env["UPDATE_PROOF_171_SUMMARY"] === "1" || !existsSync(path)) {
    writeFileSync(path, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(path, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/171/checked-summary.json is stale; rerun with UPDATE_PROOF_171_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, gate: checkedSummary.gate }));
  console.log("proof 171 passed");
}
try {
  main();
} catch (error) {

...
export function runNodeLevel5HttpResponseShapeProof(proof: string): void {
  const definition = definitions[proof];
  if (!definition) {
    throw new Error(`unknown Node Level 5 HTTP response-shape proof ${proof}`);
  }
  const checkedSummary = {
    kind: "machinen.node-level5-http-response-shape-proof-summary",
    proof,
    goal: definition.goal,
    result: definition.result,
    status: "node-product-support-80-http-response-shape-product-slice",
  ...
  console.log(JSON.stringify({ proof, httpResponseShapeGate: definition.kind }));
  console.log(`proof ${proof} node-level5 HTTP response-shape gate passed`);
}

function definitionFor(proof: number): Definition {
  if (proof <= 1248) {
    return definition(
      "HTTP response-shape product corpus generator",
      "The installed product corpus includes selected HTTP response-shape apps.",
      generatorKind(proof - 1241),
    );
  }
  if (proof <= 1256) {
    return definition(
      "HTTP response-shape product-run evidence",
      "Selected Express/Fastify cookie/status/redirect/header rows pass snapshot, restore, and behavior checks.",
      evidenceKind(proof - 1249),
    );
  }
  if (proof <= 1264) {
    return definition(
      "HTTP response-shape support matrix reconciliation",
      "Only selected response-shape rows are supported while unsafe live states remain refused.",
      matrixKind(proof - 1257),
    );
  }
  if (proof <= 1272) {
    return definition(
      "HTTP response-shape boundary preservation",
      "Arbitrary Express/Fastify apps and broad app/process support remain unclaimed.",
      boundaryKind(proof - 1265),
    );
  }
  return definition(
    "HTTP response-shape final audit",
    "The new support slice uses target-native response-shape verification without changing 80/20/0 claims.",
    auditKind(proof - 1273),
  );
}
...
function generatorKind(index: number): string {
  return [
    ...
    "runner-accepted",
    "runner-row-count",
    "runner-response-shape-row-count",
    "runner-response-shape-sources",
...
function matrixKind(index: number): string {
  return [
    ...
    "matrix-response-shape-supported",
    "matrix-response-shape-features-supported",
    "matrix-live-socket-refused",
    "matrix-response-feature-proof-range",
    "matrix-proof-range",
    "matrix-claims",
  ][index]!;
}
...
function auditKind(index: number): string {
  return [
    ...
    "audit-response-shape-not-arbitrary-framework",
...
function generatorPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    ...
    "runner-response-shape-row-count": { responseShapeRows: rows.length },
    "runner-response-shape-sources": { sources: unique(rows.map((row) => row.source)) },
...
function matrixPayload(kind: string): Record<string, unknown> {
  ...
  const selected = matrix.rows.filter((row) => responseShapeSources.includes(row.id));
  const payloads: Record<string, Record<string, unknown>> = {
    "matrix-row-count": { rowCount: matrix.rowCount },
    "matrix-supported-count": {
      supportedRows: matrix.rows.filter((row) => row.status === "supported").length,
    },
    "matrix-response-shape-supported": { ids: selected.map((row) => row.id).sort() },
    "matrix-response-shape-features-supported": {
    ...
    "matrix-response-feature-proof-range": {
      responseShapeProofRanges: unique(selected.map((row) => row.evidence.proofRange)),
    },
...
function boundaryPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    "boundary-selected-apps-only": { supportedResponseShapeSources: responseShapeSources },
...
function auditPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    ...
    "audit-response-shape-not-arbitrary-framework": { arbitraryFrameworkSupportClaimed: false },
...
function summary(): Summary {
  ...
  const provided = process.env.NODE_LEVEL5_HTTP_RESPONSE_SHAPE_SUMMARY;
  cachedSummary = provided ? JSON.parse(readFileSync(provided, "utf8")) : generateSummary();
  return cachedSummary;
}

function generateSummary(): Summary {
  const dir = mkdtempSync(join(tmpdir(), "machinen-node-level5-http-response-shape-proof-"));
  ...
  if (result.status !== 0) {
    throw new Error(
      `HTTP response-shape corpus runner failed: ${result.status} ${result.stdout} ${result.stderr}`,
    );
  }
  return JSON.parse(result.stdout);
}

function responseShapeRows(): Row[] {
  return summary().rows.filter((row: Row) => responseShapeSources.includes(row.source));
}

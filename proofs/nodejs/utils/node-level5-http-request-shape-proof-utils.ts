...
export function runNodeLevel5HttpRequestShapeProof(proof: string): void {
  const definition = definitions[proof];
  if (!definition) {
    throw new Error(`unknown Node Level 5 HTTP request-shape proof ${proof}`);
  }
  const checkedSummary = {
    kind: "machinen.node-level5-http-request-shape-proof-summary",
    proof,
    goal: definition.goal,
    result: definition.result,
    status: "node-product-support-80-http-request-shape-product-slice",
  ...
  console.log(JSON.stringify({ proof, httpRequestShapeGate: definition.kind }));
  console.log(`proof ${proof} node-level5 HTTP request-shape gate passed`);
}

function definitionFor(proof: number): Definition {
  if (proof <= 1208) {
    return definition(
      "HTTP request-shape product corpus generator",
      "The installed product corpus includes selected HTTP request-shape apps.",
      generatorKind(proof - 1201),
    );
  }
  if (proof <= 1216) {
    return definition(
      "HTTP request-shape product-run evidence",
      "Selected Express/Fastify request-shape rows pass snapshot, restore, and behavior checks.",
      evidenceKind(proof - 1209),
    );
  }
  if (proof <= 1224) {
    return definition(
      "HTTP request-shape support matrix reconciliation",
      "Only selected request-shape rows are supported while unsafe live states remain refused.",
      matrixKind(proof - 1217),
    );
  }
  if (proof <= 1232) {
    return definition(
      "HTTP request-shape boundary preservation",
      "Arbitrary Express/Fastify apps and broad app/process support remain unclaimed.",
      boundaryKind(proof - 1225),
    );
  }
  return definition(
    "HTTP request-shape final audit",
    "The new support slice uses target-native request replay verification without changing 80/20/0 claims.",
    auditKind(proof - 1233),
  );
}
...
function generatorKind(index: number): string {
  return [
    ...
    "runner-accepted",
    "runner-row-count",
    "runner-request-shape-row-count",
    "runner-request-shape-sources",
...
function matrixKind(index: number): string {
  return [
    ...
    "matrix-request-shape-supported",
    "matrix-request-shape-features-supported",
    "matrix-live-socket-refused",
    "matrix-method-feature-proof-range",
    "matrix-proof-range",
    "matrix-claims",
  ][index]!;
}
...
function auditKind(index: number): string {
  return [
    ...
    "audit-request-shape-not-arbitrary-framework",
...
function generatorPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    ...
    "runner-request-shape-row-count": { requestShapeRows: rows.length },
    "runner-request-shape-sources": { sources: unique(rows.map((row) => row.source)) },
...
function matrixPayload(kind: string): Record<string, unknown> {
  ...
  const selected = matrix.rows.filter((row) => requestShapeSources.includes(row.id));
  const payloads: Record<string, Record<string, unknown>> = {
    "matrix-row-count": { rowCount: matrix.rowCount },
    "matrix-supported-count": {
      supportedRows: matrix.rows.filter((row) => row.status === "supported").length,
    },
    "matrix-request-shape-supported": { ids: selected.map((row) => row.id).sort() },
    "matrix-request-shape-features-supported": {
    ...
    "matrix-method-feature-proof-range": {
      requestShapeProofRanges: unique(selected.map((row) => row.evidence.proofRange)),
    },
...
function boundaryPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    "boundary-selected-apps-only": { supportedRequestShapeSources: requestShapeSources },
...
function auditPayload(kind: string): Record<string, unknown> {
  ...
  const payloads: Record<string, Record<string, unknown>> = {
    ...
    "audit-request-shape-not-arbitrary-framework": { arbitraryFrameworkSupportClaimed: false },
...
function summary(): Summary {
  ...
  const provided = process.env.NODE_LEVEL5_HTTP_REQUEST_SHAPE_SUMMARY;
  cachedSummary = provided ? JSON.parse(readFileSync(provided, "utf8")) : generateSummary();
  return cachedSummary;
}

function generateSummary(): Summary {
  const dir = mkdtempSync(join(tmpdir(), "machinen-node-level5-http-request-shape-proof-"));
  ...
  if (result.status !== 0) {
    throw new Error(
      `HTTP request-shape corpus runner failed: ${result.status} ${result.stdout} ${result.stderr}`,
    );
  }
  return JSON.parse(result.stdout);
}

function requestShapeRows(): Row[] {
  return summary().rows.filter((row: Row) => requestShapeSources.includes(row.source));
}

...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/by-id/029/`. The proof smoke test may be written in TypeScript, for example `proofs/by-id/029/smoke.ts`, with an optional `proofs/by-id/029/smoke.sh` compatibility wrapper. Do not add root `package.json` scripts for this proof; run proof-local TypeScript smokes directly with `pnpm exec tsx proofs/by-id/029/smoke.ts`.

## Status

Implemented by `proofs/by-id/029/smoke.ts` and run directly with `pnpm exec tsx proofs/by-id/029/smoke.ts`.

Proof-local files:

- `proofs/by-id/029/source-app.mjs` — counter fixture captured from the source VM.
- `proofs/by-id/029/guest-capture.zig` — Zig guest capture tool matching the source-state capture shape used by prior proofs.
- `proofs/by-id/029/native-materializer.zig` — target-native materializer binary that consumes captured source-state IR and raw memory bytes, recovers the counter Smi, and emits a target-native Node trampoline.
- `proofs/by-id/029/smoke.ts` — smoke proof that runs source capture, executes the native materializer in the target VM, and verifies target `{count:3}`.
- `proofs/by-id/029/smoke.sh` — compatibility wrapper.
...
`pnpm exec tsx proofs/by-id/029/smoke.ts` now proves:
...
- `proofs/by-id/029/native-materializer.zig` runs as a target-native binary in the target VM and recovers count `2` from raw V8 context Smi memory;
...
- [x] Add proof-local materialization plan/refusal checks in `proofs/by-id/029/smoke.ts`.
- [x] Run the native materializer proof with `pnpm exec tsx proofs/by-id/029/smoke.ts`.

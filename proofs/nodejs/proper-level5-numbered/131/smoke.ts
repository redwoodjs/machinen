...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native/libuv handle inventory from live Node process",
    block: "Real libuv handle inventory",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-libuv-inventory-refused",
      actualCode: "node-proper-level5-libuv-inventory-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      libuvHandleInventoryCaptured: true,
  ...
  if (process.env.UPDATE_PROOF_131_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/131/checked-summary.json is stale; rerun with UPDATE_PROOF_131_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 131 passed");
}

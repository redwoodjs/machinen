...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/066/`. Run with `pnpm exec tsx proofs/by-id/066/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/066/smoke.ts` proves: use guest-capture-shaped output as the source of bundle sections instead of a Node mock capture tool.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/066/smoke.ts`.

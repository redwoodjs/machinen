...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Docs/support matrix for exact supported subset",
    block: "Experimental product boundary",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-supported-subset-docs-refused",
      actualCode: "node-proper-level5-supported-subset-docs-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      exactSupportedSubsetDocumented: true,
  ...
  if (process.env.UPDATE_PROOF_154_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/154/checked-summary.json is stale; rerun with UPDATE_PROOF_154_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 154 passed");
}

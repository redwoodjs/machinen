...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native heap-region scanner finds object anchors in captured V8 mappings",
    block: "Real V8 heap walker",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-v8-heap-anchor-missing",
      actualCode: "node-proper-level5-v8-heap-anchor-missing",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      nativeHeapRegionScannerFoundAnchors: true,
  ...
  if (process.env.UPDATE_PROOF_126_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/126/checked-summary.json is stale; rerun with UPDATE_PROOF_126_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 126 passed");
}

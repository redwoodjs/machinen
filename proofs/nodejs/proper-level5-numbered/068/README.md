...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/068/`. Run with `pnpm exec tsx proofs/by-id/068/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/068/smoke.ts` proves: compute section digests from artifact content instead of trusting pre-set digest-ok booleans.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/068/smoke.ts`.

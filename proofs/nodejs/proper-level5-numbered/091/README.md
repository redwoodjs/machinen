...
`pnpm exec tsx proofs/by-id/091/smoke.ts` proves guest-capture records drive native V8 byte recovery and return `{ count: 3, graphTotal: 3 }`, while missing or stale records refuse.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/091/smoke.ts`.

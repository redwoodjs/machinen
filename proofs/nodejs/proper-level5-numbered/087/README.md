...
`pnpm exec tsx proofs/by-id/087/smoke.ts` proves the proof-local arm64-evidence to amd64-target pipeline starts target-native Node after native verification and returns the next state.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/087/smoke.ts`.

...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/074/`. Run with `pnpm exec tsx proofs/by-id/074/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/074/smoke.ts` proves: capture all procfs-shaped thread entries for a live proof process and classify the whole process.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/074/smoke.ts`.

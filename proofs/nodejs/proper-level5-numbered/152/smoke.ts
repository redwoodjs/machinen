...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Real guarded restore command path behind feature flag",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-guarded-restore-refused",
      actualCode: "node-proper-level5-guarded-restore-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      guardedRestoreCommandPathExercised: true,
  ...
  if (process.env.UPDATE_PROOF_152_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/152/checked-summary.json is stale; rerun with UPDATE_PROOF_152_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 152 passed");
}

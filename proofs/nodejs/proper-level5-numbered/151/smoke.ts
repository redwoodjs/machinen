...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Real guarded capture command path behind feature flag",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-guarded-capture-refused",
      actualCode: "node-proper-level5-guarded-capture-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      guardedCaptureCommandPathExercised: true,
  ...
  if (process.env.UPDATE_PROOF_151_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/151/checked-summary.json is stale; rerun with UPDATE_PROOF_151_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 151 passed");
}

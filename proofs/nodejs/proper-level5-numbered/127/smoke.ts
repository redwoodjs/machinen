...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Decode real string and array object records from captured bytes",
    block: "Real V8 string/array decoding",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-v8-string-array-bytes-refused",
      actualCode: "node-proper-level5-v8-string-array-bytes-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      realStringArrayRecordsDecoded: true,
  ...
  if (process.env.UPDATE_PROOF_127_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/127/checked-summary.json is stale; rerun with UPDATE_PROOF_127_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 127 passed");
}

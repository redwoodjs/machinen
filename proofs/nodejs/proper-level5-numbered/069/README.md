...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/069/`. Run with `pnpm exec tsx proofs/by-id/069/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/069/smoke.ts` proves: emit stable refusal reports with section, field, code, and evidence path.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/069/smoke.ts`.

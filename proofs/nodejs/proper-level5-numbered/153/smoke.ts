...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Stable public refusal error registry",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-refusal-registry-refused",
      actualCode: "node-proper-level5-refusal-registry-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      stableRefusalErrorRegistryRecorded: true,
  ...
  if (process.env.UPDATE_PROOF_153_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/153/checked-summary.json is stale; rerun with UPDATE_PROOF_153_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 153 passed");
}

...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native verifier parses real process/maps/fd/thread/resource bundle",
    block: "Native end-to-end bundle parser",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-native-bundle-parse-refused",
      actualCode: "node-proper-level5-native-bundle-parse-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      nativeVerifierParsedFullBundle: true,
  ...
  if (process.env.UPDATE_PROOF_136_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/136/checked-summary.json is stale; rerun with UPDATE_PROOF_136_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 136 passed");
}

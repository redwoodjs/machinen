...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/078/`. Run with `pnpm exec tsx proofs/by-id/078/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/078/smoke.ts` proves: emit separate timer, eventfd, and pipe descriptors from their own evidence records.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/078/smoke.ts`.

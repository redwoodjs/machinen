...
`pnpm exec tsx proofs/by-id/086/smoke.ts` proves native byte recovery returns `{ count: 3, graphTotal: 3 }` for the accepted row and refuses malformed byte artifacts before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/086/smoke.ts`.

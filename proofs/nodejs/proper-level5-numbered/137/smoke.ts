...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native verifier validates Node/V8/libuv build identity",
    block: "Native build identity gate",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-native-build-identity-refused",
      actualCode: "node-proper-level5-native-build-identity-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      nativeBuildIdentityGateValidated: true,
  ...
  if (process.env.UPDATE_PROOF_137_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/137/checked-summary.json is stale; rerun with UPDATE_PROOF_137_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 137 passed");
}

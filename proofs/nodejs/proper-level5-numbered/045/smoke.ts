...
function auditProof(id: string): Record<string, unknown> {
  const readmePath = join(repoRoot, "proofs", "by-id", id, "README.md");
  const summaryPath = join(repoRoot, "proofs", "by-id", id, "checked-summary.json");
...
function main(): void {
  ...
  if (process.env.UPDATE_PROOF_045_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/045/checked-summary.json is stale; rerun with UPDATE_PROOF_045_SUMMARY=1",
    );
  }

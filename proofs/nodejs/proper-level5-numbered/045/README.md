...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/by-id/045/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/by-id/045/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/045/smoke.ts` now audits Proofs 027–044 and proves:
...
- [x] Run `pnpm exec tsx proofs/by-id/045/smoke.ts`.

...
function runProof100(index: number): RunSummary {
  const run = spawnSync("pnpm", ["exec", "tsx", "proofs/by-id/100/smoke.ts"], {
    cwd: repoRoot,
    encoding: "utf8",
  ...
  return JSON.parse(
    readFileSync(join(repoRoot, "proofs/by-id/100/checked-summary.json"), "utf8"),
  ) as RunSummary;
}
...
function main(): void {
  ...
  const checkedSummary = {
    ...
    productSupportClaimed: false,
    broadLevel5ImplementationClaimed: false,
    automationLane: "pnpm exec tsx proofs/by-id/100/smoke.ts repeated twice",
  ...
  if (process.env.UPDATE_PROOF_103_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/103/checked-summary.json is stale; rerun with UPDATE_PROOF_103_SUMMARY=1",
    );
  }

...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Express-style HTTP counter app",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-real-app-http-refused",
      actualCode: "node-proper-level5-real-app-http-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      expressStyleHttpCounterContinued: true,
  ...
  if (process.env.UPDATE_PROOF_141_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/141/checked-summary.json is stale; rerun with UPDATE_PROOF_141_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 141 passed");
}

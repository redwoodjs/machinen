...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Artifact diffing with stable normalized summaries",
    block: "Automation artifact diffing",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-artifact-diff-refused",
      actualCode: "node-proper-level5-artifact-diff-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      normalizedArtifactDiffStable: true,
  ...
  if (process.env.UPDATE_PROOF_148_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/148/checked-summary.json is stale; rerun with UPDATE_PROOF_148_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 148 passed");
}

...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "TCP listener + pipe/stdio app",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-real-app-pipe-refused",
      actualCode: "node-proper-level5-real-app-pipe-refused",
      targetStarted: false,
    },
    {
  ...
  if (process.env.UPDATE_PROOF_143_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/143/checked-summary.json is stale; rerun with UPDATE_PROOF_143_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 143 passed");
}

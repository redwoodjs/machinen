...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/by-id/046/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/by-id/046/smoke.ts`; do not add a root `package.json` script.
...
- [x] Keep proof-local smokes under `proofs/by-id/046/` with no root package script.

## Proof result

`pnpm exec tsx proofs/by-id/046/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proofs/by-id/046/smoke.ts`.

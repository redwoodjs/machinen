...
function main(): void {
  ...
  const capture = spawnSync(
    "node",
    [join(repoRoot, "proofs/by-id/056/capture-tool.mjs"), artifacts],
    {
      encoding: "utf8",
    },
  );
  ...
  spawnSync(
    ...
    [
      "run",
      join(repoRoot, "proofs/by-id/058/native-bundle-assembler.zig"),
      "--",
      artifacts,
      nativeBundlePath,
      assemblerResultPath,
  ...
  const refusedRows = [
    ...
    spawnSync(
      "zig",
      ["run", join(repoRoot, "proofs/by-id/057/native-json-verifier.zig"), "--", path, resultPath],
      { encoding: "utf8", maxBuffer: 64 * 1024 * 1024 },
    );
  ...
  if (process.env.UPDATE_PROOF_059_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/059/checked-summary.json is stale; rerun with UPDATE_PROOF_059_SUMMARY=1",
    );
  }

...
`pnpm exec tsx proofs/by-id/092/smoke.ts` proves a supported Node 22 / V8 12 style record reconstructs `{ total: 3, history: [1, 2, 3] }`, while unsupported builds, encodings, and maps refuse.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/092/smoke.ts`.

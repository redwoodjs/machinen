...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native verifier emits signed/hashed continuation descriptor",
    block: "Signed continuation descriptor",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-native-signature-refused",
      actualCode: "node-proper-level5-native-signature-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      signedContinuationDescriptorEmitted: true,
  ...
  if (process.env.UPDATE_PROOF_140_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/140/checked-summary.json is stale; rerun with UPDATE_PROOF_140_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 140 passed");
}

...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/067/`. Run with `pnpm exec tsx proofs/by-id/067/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/067/smoke.ts` proves: feed guest-capture artifacts to a native assembler and refuse missing or stale artifact inputs.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/067/smoke.ts`.

...
function runProof(proof: string): Record<string, unknown> {
  const run = spawnSync("pnpm", ["exec", "tsx", `proofs/by-id/${proof}/smoke.ts`], {
    cwd: repoRoot,
    encoding: "utf8",
...
function main(): void {
  ...
  const checkedSummary = {
    ...
    automationLane: "repeat proofs/115 and proofs/123 twice and compare normalized artifacts",
    artifactDiff,
    assertions: {
  ...
  if (process.env.UPDATE_PROOF_124_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/124/checked-summary.json is stale; rerun with UPDATE_PROOF_124_SUMMARY=1",
    );
  }

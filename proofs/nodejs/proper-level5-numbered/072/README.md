...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/072/`. Run with `pnpm exec tsx proofs/by-id/072/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/072/smoke.ts` proves: require every decoded object edge to point back to captured map or hidden-class evidence.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/072/smoke.ts`.

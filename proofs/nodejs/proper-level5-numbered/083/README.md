...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/083/`. Run with `pnpm exec tsx proofs/by-id/083/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/083/smoke.ts` proves: allow target materialization only with an explicit proof-only start flag after all gates pass.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/083/smoke.ts`.

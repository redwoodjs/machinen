...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/084/`. Run with `pnpm exec tsx proofs/by-id/084/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/084/smoke.ts` proves: mirror the direction so amd64 source evidence can plan/materialize arm64 target-native state.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/084/smoke.ts`.

...
`pnpm exec tsx proofs/by-id/057/smoke.ts` proves the native verifier parses structured JSON, accepts the valid bundle, and refuses malformed JSON, bad schema, bad architecture, bad continuation, digest failure, product claims, and shortcut flags before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/057/smoke.ts`.

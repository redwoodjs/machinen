...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native verifier validates thread/fd/resource consistency",
    block: "Native consistency gate",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-native-consistency-refused",
      actualCode: "node-proper-level5-native-consistency-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      nativeThreadFdResourceConsistencyValidated: true,
  ...
  if (process.env.UPDATE_PROOF_138_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/138/checked-summary.json is stale; rerun with UPDATE_PROOF_138_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 138 passed");
}

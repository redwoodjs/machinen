...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Decode closure contexts from captured V8 context slots",
    block: "Real V8 closure context decoding",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-v8-context-slot-refused",
      actualCode: "node-proper-level5-v8-context-slot-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      closureContextSlotsDecoded: true,
  ...
  if (process.env.UPDATE_PROOF_129_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/129/checked-summary.json is stale; rerun with UPDATE_PROOF_129_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 129 passed");
}

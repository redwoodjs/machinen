...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/by-id/026/`. The proof smoke test may be written in TypeScript, for example `proofs/by-id/026/smoke.ts`, with an optional `proofs/by-id/026/smoke.sh` compatibility wrapper. Do not add root `package.json` scripts for this proof; run proof-local TypeScript smokes directly with `pnpm exec tsx proofs/by-id/026/smoke.ts`.

## Status

Implemented by `proofs/by-id/026/smoke.ts` and run directly with `pnpm exec tsx proofs/by-id/026/smoke.ts`. The source app, Zig guest capture tool, and target loader live beside it as proof-local fixture files.
...
`pnpm exec tsx proofs/by-id/026/smoke.ts` now proves:

- source Node returns `{ "total": 1, "history": [1] }`, then `{ "total": 2, "history": [1, 2] }`;
- capture is external (`SIGSTOP`) using `proofs/by-id/026/guest-capture.zig` and records `/proc`, memory, fd/socket, V8, and object-state evidence;
...
- [x] Run `pnpm exec tsx proofs/by-id/026/smoke.ts`.

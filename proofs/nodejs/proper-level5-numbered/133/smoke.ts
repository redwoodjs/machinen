...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "TCP listener reconstruction from real socket/libuv handle pairing",
    block: "Real TCP/libuv pairing",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-libuv-socket-pairing-refused",
      actualCode: "node-proper-level5-libuv-socket-pairing-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      tcpListenerReconstructedFromSocketPairing: true,
  ...
  if (process.env.UPDATE_PROOF_133_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/133/checked-summary.json is stale; rerun with UPDATE_PROOF_133_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 133 passed");
}

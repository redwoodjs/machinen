...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Pending req/handle refusal based on real libuv queues",
    block: "Real libuv pending-work refusal",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-libuv-pending-queue-refused",
      actualCode: "node-proper-level5-libuv-pending-queue-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      pendingLibuvQueuesRefused: true,
  ...
  if (process.env.UPDATE_PROOF_135_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/135/checked-summary.json is stale; rerun with UPDATE_PROOF_135_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 135 passed");
}

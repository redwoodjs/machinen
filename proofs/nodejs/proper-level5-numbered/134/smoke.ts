...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Pipe/file/stdio reconstruction from real fd + libuv records",
    block: "Real fd/libuv resource reconstruction",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-libuv-fd-record-refused",
      actualCode: "node-proper-level5-libuv-fd-record-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      pipeFileStdioReconstructedFromFdRecords: true,
  ...
  if (process.env.UPDATE_PROOF_134_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/134/checked-summary.json is stale; rerun with UPDATE_PROOF_134_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 134 passed");
}

...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Decode real plain object properties from hidden-class/map evidence",
    block: "Real V8 plain object decoding",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-v8-plain-object-map-refused",
      actualCode: "node-proper-level5-v8-plain-object-map-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      plainObjectPropertiesDecodedFromMapEvidence: true,
  ...
  if (process.env.UPDATE_PROOF_128_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/128/checked-summary.json is stale; rerun with UPDATE_PROOF_128_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 128 passed");
}

...
function main(): void {
  ...
  const ciProofLane = {
    ...
    commands: ["pnpm exec tsx proofs/by-id/089/smoke.ts", "pnpm run format:check", "pnpm run lint"],
  };
  ...
  if (process.env.UPDATE_PROOF_089_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/089/checked-summary.json is stale; rerun with UPDATE_PROOF_089_SUMMARY=1",
    );
  }

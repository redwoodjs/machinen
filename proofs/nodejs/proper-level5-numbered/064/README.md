...
`pnpm exec tsx proofs/by-id/064/smoke.ts` proves every missing gate refuses before target start, while the all-gates-passed row starts target-native materialization and returns `{ count: 3, graphTotal: 3 }`.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/064/smoke.ts`.

...
function verify(work: string, id: string, evidence: Record<string, unknown>): VerifyResult {
  ...
  spawnSync(
    ...
    [
      "run",
      join(repoRoot, "proofs/by-id/093/native-process-resource-verifier.zig"),
      "--",
      path,
      resultPath,
    ],
...
function main(): void {
  ...
  if (process.env.UPDATE_PROOF_101_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/101/checked-summary.json is stale; rerun with UPDATE_PROOF_101_SUMMARY=1",
    );
  }

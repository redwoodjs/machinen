...
`pnpm exec tsx proofs/by-id/101/smoke.ts` proves the native process/resource verifier consumes capture-derived evidence and refuses unsafe rows before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/101/smoke.ts`.

...
const work =
  ...
  mkdtempSync(join(process.env.TMPDIR ?? tmpdir(), "machinen-node-proper-level5-v8-heap-graph."));
const activeSource = `node-proper-level5-v8-heap-graph-active-source-${process.pid}`;
const busySource = `node-proper-level5-v8-heap-graph-busy-source-${process.pid}`;
const blockingSource = `node-proper-level5-v8-heap-graph-blocking-source-${process.pid}`;
const unsupportedSource = `node-proper-level5-v8-heap-graph-unsupported-source-${process.pid}`;
const idleSource = `node-proper-level5-v8-heap-graph-idle-source-${process.pid}`;
const targetName = `node-proper-level5-v8-heap-graph-target-${process.pid}`;
...
interface CaptureMarkers {
  ...
  unsupportedV8ProxyDetected: boolean;
  graphAnchorFound: boolean;
...
interface ProcessImageInventory {
  kind: "machinen.node-proper-level5-v8-heap-graph-inventory";
...
function assertGraph(body: string, expectedTotal: number): void {
  const parsed = parseJson<GraphResponse>(body);
  if (parsed.total !== expectedTotal) {
    throw new Error(`expected total ${expectedTotal}, got ${body}`);
  }
  if (parsed.historyLength !== expectedTotal) {
    throw new Error(`expected history length ${expectedTotal}, got ${body}`);
  }
  if (!parsed.leftSharedIsRightShared || !parsed.packedSharedIsSame) {
    throw new Error(`expected shared object identity evidence, got ${body}`);
  }
  if (parsed.sharedHits !== expectedTotal) {
    throw new Error(`expected shared hits ${expectedTotal}, got ${body}`);
  }
  if (parsed.name !== "graph-alpha") {
    throw new Error(`expected graph name evidence, got ${body}`);
  }
}
...
function bootSource(name: VmName): void {
  ...
  copyFileSync(join(proofDir, "source-app.mjs"), join(work, "server-033.mjs"));
  execInVm(
    name,
    "rm -f /tmp/machinen-proof-033-fifo && mkfifo /tmp/machinen-proof-033-fifo && mkdir -p /opt/machinen-proof-033 && cp /mnt/work/server-033.mjs /opt/machinen-proof-033/server-033.mjs && cd /opt/machinen-proof-033 && nohup node --v8-pool-size=0 --single-threaded --single-threaded-gc server-033.mjs >/tmp/node-proof-033.log 2>&1 &",
  );
}
...
async function waitForHttp(vmName: VmName): Promise<string> {
  ...
  process.stderr.write(execInVm(vmName, "cat /tmp/node-proof-033.log || true"));
...
function findSourcePid(vmName: VmName): string {
  const pid = execInVm(
    vmName,
    "for p in /proc/[0-9]*; do exe=$(readlink $p/exe 2>/dev/null || true); case $exe in */node) tr '\\0' ' ' < $p/cmdline 2>/dev/null | grep -q server-033.mjs && basename $p && break;; esac; done",
  ).trim();
  if (!pid) {
    process.stderr.write(execInVm(vmName, "ps -ef || true; cat /tmp/node-proof-033.log || true"));
...
async function runGuestCaptureWithVmPauseBarrier(
  ...
  execInVm(
    vmName,
    `rm -rf /mnt/work/${label}-state /mnt/work/${label}-barrier && mkdir -p /mnt/work/${label}-barrier && chmod +x /mnt/work/${binary} && sh -c '(/mnt/work/${binary} /mnt/work/${label}-state ${pid} /mnt/work/${label}-barrier >/tmp/node-proof-033-${label}-capture.log 2>&1; echo $? >/mnt/work/${label}-barrier/status) &'`,
  );
  ...
  if (status !== "0") {
    process.stderr.write(execInVm(vmName, `cat /tmp/node-proof-033-${label}-capture.log || true`));
...
function buildProcessImageInventory(
  ...
  writeFileSync(
    join(captureRoot, "v8-heap-graph-inventory.json"),
...
function classifyThreadContinuations(
  ...
  if (markers.unsupportedV8ProxyDetected) {
    failures.push({
      code: "node-proper-level5-v8-unsupported-proxy-or-sparse-array",
      message: "unsupported V8 proxy or sparse array shape was detected in captured heap evidence",
      continuationClass: "v8-internal-frame",
    });
  }
  ...
  const threadClasses = inventory.threads.map((thread) => ({
    ...
    classificationEvidence: accepted
      ? "no active HTTP, JavaScript callback, unsupported syscall, or unsupported V8 shape marker found"
      : "refusal marker found in captured source memory",
  }));
  const descriptors = accepted
    ? threadClasses.map((thread) => ({
        kind: "machinen.v8-heap-graph-descriptor",
        tid: thread.tid,
...
function buildSummary(label: string, pauseEvidence: Record<string, unknown>): void {
  ...
  const summary = {
    kind: "machinen.node-proper-level5-v8-heap-graph-source-state-capture",
    goal: "proper-node-level5-v8-heap-graph-atomic-source-capture-proof",
    pid: markers.pid,
    externalQuiesce: {
      method: "v8-heap-graph-barrier+process-freeze",
      appHookUsed: false,
    ...
    captured: {
      ...
      guestCaptureTool: "proofs/by-id/033/guest-capture.zig",
    },
    classification: {
      ...
      unsupportedV8ProxyDetected: markers.unsupportedV8ProxyDetected,
      graphAnchorFound: markers.graphAnchorFound,
      acceptedMappings,
      processImageInventoryPath: "v8-heap-graph-inventory.json",
      processImageInventory,
    ...
    runtimeStateCandidates: {
      ...
      v8SupportedHeapGraphCandidates: markers.graphAnchorFound
        ? [{ kind: "v8-supported-plain-object-array-string-graph-candidate" }]
        : [],
    ...
    portableIr: {
      ...
      memoryObjectGraphFragments: markers.graphAnchorFound
        ? [{ kind: "v8-supported-plain-object-array-string-graph-candidate" }]
        : [],
...
async function proveActiveRequestRefusal(): Promise<void> {
  ...
  execInVm(
    activeSource,
    "nohup curl -fsS http://127.0.0.1:3000/hold >/tmp/node-proof-033-hold.out 2>&1 &",
  );
...
async function proveBusyJsRefusal(): Promise<void> {
  ...
  execInVm(
    busySource,
    "nohup curl --max-time 45 -fsS http://127.0.0.1:3000/busy >/tmp/node-proof-033-busy.out 2>&1 &",
  );
  await sleep(750);
...
async function proveBlockingSyscallRefusal(): Promise<void> {
  ...
  execInVm(
    blockingSource,
    "nohup curl --max-time 45 -fsS http://127.0.0.1:3000/blocking-syscall >/tmp/node-proof-033-blocking-syscall.out 2>&1 &",
  );
  await sleep(750);
...
async function proveUnsupportedV8ShapeRefusal(): Promise<void> {
  bootSource(unsupportedSource);
  await waitForHttp(unsupportedSource);
  execInVm(unsupportedSource, "curl -fsS http://127.0.0.1:3000/unsupported >/dev/null");
  await runGuestCaptureWithVmPauseBarrier(unsupportedSource, "unsupported");
  assertRefused("unsupported", "node-proper-level5-v8-unsupported-proxy-or-sparse-array");
}
...
async function proveIdleContinuation(): Promise<void> {
  ...
  assertGraph(sourceOne, 1);
  assertGraph(sourceTwo, 2);
  const pauseEvidence = await runGuestCaptureWithVmPauseBarrier(idleSource, "idle");
  const resumedState = curl(idleSource, "/state");
  assertGraph(resumedState, 2);

  bootTarget();
  copyFileSync(join(proofDir, "target-loader.mjs"), join(work, "target-loader.mjs"));
  execInVm(
    targetName,
    "nohup node /mnt/work/target-loader.mjs /mnt/work/idle-state /mnt/work/proof-result.json >/tmp/node-proof-033-target.log 2>&1 &",
  );
  ...
  if (!targetOne) {
    process.stderr.write(execInVm(targetName, "cat /tmp/node-proof-033-target.log || true"));
    throw new Error("target did not start");
  }
  assertGraph(targetOne, 3);
  ...
  const inventory = parseJson<ProcessImageInventory>(
    readFileSync(join(work, "idle-state/v8-heap-graph-inventory.json"), "utf8"),
  );
  ...
  if (!proof.identityPreserved) {
    throw new Error("target heap graph did not preserve shared-reference identity");
  }
  if (
    (proof.heapGraphIr as { priorJsonResponseStringsUsed?: boolean }).priorJsonResponseStringsUsed
  ) {
    throw new Error("target heap graph used prior JSON response strings as source of truth");
  }
  for (const key of [
...
try {
  ...
  await proveUnsupportedV8ShapeRefusal();
  await proveIdleContinuation();
  console.log(`node proper Level 5 v8-heap-graph capture proof passed: ${work}`);
} catch (error) {
  console.error(error);
  process.exitCode = 1;
}

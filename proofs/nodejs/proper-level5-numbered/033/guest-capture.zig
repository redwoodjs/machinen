...
const unsupported_proxy_marker = "machinen-level5-v8-unsupported-proxy-v1";
const graph_anchor = "machinen-level5-v8-heap-graph-anchor-v1";
...
pub fn main(init: std.process.Init) !u8 {
    ...
    var unsupported_proxy_found = false;
    var graph_anchor_found = false;
    ...
    while (line_it.next()) |line| : (map_index += 1) {
        ...
        unsupported_proxy_found = unsupported_proxy_found or std.mem.indexOf(u8, got, unsupported_proxy_marker) != null;
        graph_anchor_found = graph_anchor_found or std.mem.indexOf(u8, got, graph_anchor) != null;
        counter_anchor_found = counter_anchor_found or std.mem.indexOf(u8, got, counter_anchor) != null;
        source_found = source_found or std.mem.indexOf(u8, got, "machinenHeapGraphRetainer") != null;
    }
    try write_out(allocator, out_root, "accepted-mappings.tsv", accepted.items);

    const markers = try std.fmt.allocPrint(allocator,
        \\
        \\{{"pid":{d},"activeHttpRequestDetected":{},"activeJsCallbackDetected":{},"activeBlockingSyscallDetected":{},"unsupportedV8ProxyDetected":{},"graphAnchorFound":{},"counterAnchorFound":{},"sourceCounterTextFound":{},"acceptedMappings":{d}}}
        \\
    , .{ pid, active_found, busy_js_found, blocking_syscall_found, unsupported_proxy_found, graph_anchor_found, counter_anchor_found, source_found, accepted_count });

...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "arm64→amd64 real capture E2E repeated N times",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-arm64-amd64-repeatability-refused",
      actualCode: "node-proper-level5-arm64-amd64-repeatability-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      arm64ToAmd64RepeatedE2eStable: true,
  ...
  if (process.env.UPDATE_PROOF_146_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/146/checked-summary.json is stale; rerun with UPDATE_PROOF_146_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 146 passed");
}

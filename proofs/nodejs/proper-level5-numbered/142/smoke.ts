...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "HTTP + timer + file config app",
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-real-app-file-config-refused",
      actualCode: "node-proper-level5-real-app-file-config-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      httpTimerFileConfigContinued: true,
  ...
  if (process.env.UPDATE_PROOF_142_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/142/checked-summary.json is stale; rerun with UPDATE_PROOF_142_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 142 passed");
}

...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Negative real-app suite for active/unsupported state",
    block: "Real app negative suite",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-real-app-negative-suite-refused",
      actualCode: "node-proper-level5-real-app-negative-suite-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      negativeRealAppSuiteRefusedUnsafeNeighbors: true,
  ...
  if (process.env.UPDATE_PROOF_145_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/145/checked-summary.json is stale; rerun with UPDATE_PROOF_145_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 145 passed");
}

...
function runThread(work: string, id: string, lines: string): Result {
  ...
  spawnSync(
    "zig",
    ["run", join(repoRoot, "proofs/by-id/116/native-thread-set-verifier.zig"), "--", input, output],
...
function runResource(work: string, id: string, lines: string): Result {
  ...
  spawnSync(
    ...
    [
      "run",
      join(repoRoot, "proofs/by-id/119/native-kernel-resource-verifier.zig"),
      "--",
      input,
      output,
    ],
...
function main(): void {
  ...
  if (process.env.UPDATE_PROOF_120_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/by-id/120/checked-summary.json is stale; rerun with UPDATE_PROOF_120_SUMMARY=1",
    );
  }

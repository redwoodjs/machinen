...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Native verifier validates heap graph references and resource links",
    block: "Native graph/resource link gate",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-native-link-refused",
      actualCode: "node-proper-level5-native-link-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      nativeHeapResourceLinksValidated: true,
  ...
  if (process.env.UPDATE_PROOF_139_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/139/checked-summary.json is stale; rerun with UPDATE_PROOF_139_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 139 passed");
}

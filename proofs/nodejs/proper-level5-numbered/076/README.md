...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/076/`. Run with `pnpm exec tsx proofs/by-id/076/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/076/smoke.ts` proves: walk live fd-table-shaped evidence and emit descriptors with fd provenance.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/076/smoke.ts`.

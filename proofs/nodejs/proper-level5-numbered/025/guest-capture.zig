...
const app_cmdline_marker = "counter.mjs";
const counter_anchor = "machinen-level5-v8-context-anchor-v1";
const timer_anchor = "machinen-level5-libuv-timer-anchor-v1";
const source_counter_text = "let count = 0;";
const count_response_prefix = "{\"count\":";
...
pub fn main(init: std.process.Init) !u8 {
    ...
    var timer_anchor_found = false;
    var source_found = false;
    var count_response_found = false;
    ...
    while (line_it.next()) |line| : (map_index += 1) {
        ...
        timer_anchor_found = timer_anchor_found or std.mem.indexOf(u8, got, timer_anchor) != null;
        source_found = source_found or std.mem.indexOf(u8, got, source_counter_text) != null;
        count_response_found = count_response_found or std.mem.indexOf(u8, got, count_response_prefix) != null;
    }
    try write_out(allocator, out_root, "accepted-mappings.tsv", accepted.items);

    const markers = try std.fmt.allocPrint(allocator,
        \\
        \\{{"pid":{d},"counterAnchorFound":{},"timerAnchorFound":{},"sourceCounterTextFound":{},"countResponseFound":{},"acceptedMappings":{d}}}
        \\
    , .{ pid, counter_anchor_found, timer_anchor_found, source_found, count_response_found, accepted_count });

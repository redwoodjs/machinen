...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/082/`. Run with `pnpm exec tsx proofs/by-id/082/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/082/smoke.ts` proves: wire the private CLI proof path through native assembly and structured verification together.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/082/smoke.ts`.

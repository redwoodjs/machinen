...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Closure-heavy app with arrays/objects/strings",
    block: "Real app suite",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-real-app-closure-graph-refused",
      actualCode: "node-proper-level5-real-app-closure-graph-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      closureHeavyGraphAppContinued: true,
  ...
  if (process.env.UPDATE_PROOF_144_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/144/checked-summary.json is stale; rerun with UPDATE_PROOF_144_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 144 passed");
}

...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/by-id/077/`. Run with `pnpm exec tsx proofs/by-id/077/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/by-id/077/smoke.ts` proves: derive listener address, port, state, and queue policy from proc-net TCP evidence.

## Validation

- [x] Run `pnpm exec tsx proofs/by-id/077/smoke.ts`.

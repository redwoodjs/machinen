...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Failure triage artifacts for every refusal class",
    block: "Automation failure triage",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-failure-triage-refused",
      actualCode: "node-proper-level5-failure-triage-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      failureTriageArtifactsWritten: true,
  ...
  if (process.env.UPDATE_PROOF_149_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/149/checked-summary.json is stale; rerun with UPDATE_PROOF_149_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 149 passed");
}

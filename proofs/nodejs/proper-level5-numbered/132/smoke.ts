...
function main(): void {
  ...
  const accepted = {
    proof,
    title: "Timer handle reconstruction from real libuv timer fields",
    block: "Real libuv timer reconstruction",
    targetStarted: false,
  ...
  const refusedRows: RefusedRow[] = [
    {
      ...
      expectedCode: "node-proper-level5-libuv-timer-field-refused",
      actualCode: "node-proper-level5-libuv-timer-field-refused",
      targetStarted: false,
    },
    {
  ...
  const checkedSummary = {
    ...
    refusedRows,
    assertions: {
      timerReconstructedFromLibuvFields: true,
  ...
  if (process.env.UPDATE_PROOF_132_SUMMARY === "1" || !existsSync(summaryPath)) {
    writeFileSync(summaryPath, text);
  } else if (
    JSON.stringify(JSON.parse(readFileSync(summaryPath, "utf8"))) !== JSON.stringify(checkedSummary)
  ) {
    throw new Error(
      "proofs/by-id/132/checked-summary.json is stale; rerun with UPDATE_PROOF_132_SUMMARY=1",
    );
  }
  console.log(JSON.stringify({ proof, title: accepted.title, refused: refusedRows.length }));
  console.log("proof 132 passed");
}

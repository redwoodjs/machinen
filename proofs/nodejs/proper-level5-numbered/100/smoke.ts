...
async function main(): Promise<void> {
  ...
  runZig(["run", join(repoRoot, "proofs/by-id/096/guest-capture-records.zig"), "--", work]);
  const parseResultPath = join(work, "parse-result.json");
  runZig([
    "run",
    join(repoRoot, "proofs/by-id/097/native-record-parser.zig"),
    "--",
    work,
    parseResultPath,
  ]);
  ...
  runZig([
    "run",
    join(repoRoot, "proofs/by-id/086/native-v8-byte-decoder.zig"),
    "--",
  ...
  runZig([
    "run",
    join(repoRoot, "proofs/by-id/057/native-json-verifier.zig"),
    "--",
    bundlePath,
    verifierResultPath,
  ]);
  ...
  writeFileSync(
    ...
    readFileSync(join(repoRoot, "proofs/by-id/087/target-loader.mjs")),
  );
  ...
  try {
    ...
    if (process.env.UPDATE_PROOF_100_SUMMARY === "1" || !existsSync(summaryPath)) {
      ...
      throw new Error(
        "proofs/by-id/100/checked-summary.json is stale; rerun with UPDATE_PROOF_100_SUMMARY=1",
      );
    }

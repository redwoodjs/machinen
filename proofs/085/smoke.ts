...
function auditProof(proof: string): Record<string, unknown> {
  const readmePath = join(repoRoot, "proofs", proof, "README.md");
  const summaryPath = join(repoRoot, "proofs", proof, "checked-summary.json");
...
function main(): void {
  ...
  if (process.env.UPDATE_PROOF_085_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/085/checked-summary.json is stale; rerun with UPDATE_PROOF_085_SUMMARY=1",
    );
  }
  console.log(
    JSON.stringify({

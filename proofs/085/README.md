...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/085/`. Run with `pnpm exec tsx proofs/085/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/085/smoke.ts` proves Proofs 066–084 and public docs keep proof-only boundaries and do not advertise broad translated-continuation product support.

## Validation

- [x] Run `pnpm exec tsx proofs/085/smoke.ts`.

...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/052/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/052/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/052/smoke.ts` proves expanded supported resource descriptors materialize as fresh target-native handles and unsafe/ambiguous resources refuse before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/052/smoke.ts`.

...
`pnpm exec tsx proofs/100/smoke.ts` proves Zig guest capture → native parse → native V8 decode → native verify → amd64 target response `{ count: 3, graphTotal: 3 }`.

## Validation

- [x] Run `pnpm exec tsx proofs/100/smoke.ts`.

...
`pnpm exec tsx proofs/059/smoke.ts` proves the proof-local end-to-end flow reaches `{ count: 3, graphTotal: 3 }` and refuses unsafe cross-arch variants before target start.

## Validation

- [x] Run `pnpm exec tsx proofs/059/smoke.ts`.

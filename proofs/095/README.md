...
`pnpm exec tsx proofs/095/smoke.ts` proves the experimental CLI dry-run accepts only with explicit flags, refuses unsafe cases with clear messages, and keeps the support matrix not-supported.

## Validation

- [x] Run `pnpm exec tsx proofs/095/smoke.ts`.

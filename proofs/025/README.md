...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/025/`. Proof smoke tests may be written in TypeScript. This proof is implemented at `proofs/025/smoke.ts`; `proofs/025/smoke.sh` is only a compatibility wrapper. Do not add root `package.json` scripts for this proof; run it directly with `pnpm exec tsx proofs/025/smoke.ts`.

## Status

Implemented by `proofs/025/smoke.ts` and run directly with `pnpm exec tsx proofs/025/smoke.ts`. The source app, Zig guest capture tool, and target loader live beside it as proof-local fixture files.
...
`pnpm exec tsx proofs/025/smoke.ts` now proves:

- source Node returns `{ "count": 1 }`, `{ "count": 2 }`;
- source timer state is observed before capture;
- capture is external (`SIGSTOP`) using `proofs/025/guest-capture.zig` and records `/proc`, memory, fd/socket, V8, and timer evidence;
...
- [x] Run `pnpm exec tsx proofs/025/smoke.ts`.

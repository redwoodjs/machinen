...
All implementation notes, fixtures, and smoke tests for this proof should live under `proofs/043/`. Run the proof-local TypeScript smoke directly with `pnpm exec tsx proofs/043/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/043/smoke.ts` now proves:
...
- [x] Run `pnpm exec tsx proofs/043/smoke.ts`.

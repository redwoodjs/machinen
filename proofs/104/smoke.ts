...
function main(): void {
  const proof100 = spawnSync("pnpm", ["exec", "tsx", "proofs/100/smoke.ts"], {
    cwd: repoRoot,
    encoding: "utf8",
    maxBuffer: 64 * 1024 * 1024,
  });
  ...
  const accepted = runCli([
    ...
    "--proof-100-summary",
    "proofs/100/checked-summary.json",
  ]);
  ...
  const cases: Array<[string, string[], string]> = [
    [
      "missing-flags",
      ["--proof-100-summary", "proofs/100/checked-summary.json"],
  ...
  const checkedSummary = {
    ...
    accepted,
    refusedRows,
    docs: "proofs/104/EXPERIMENTAL.md",
    assertions: {
  ...
  if (process.env.UPDATE_PROOF_104_SUMMARY === "1" || !existsSync(summaryPath)) {
    ...
    throw new Error(
      "proofs/104/checked-summary.json is stale; rerun with UPDATE_PROOF_104_SUMMARY=1",
    );
  }

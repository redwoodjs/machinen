...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/070/`. Run with `pnpm exec tsx proofs/070/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/070/smoke.ts` proves: let the target materializer proceed only from verifier output, not raw unchecked bundle input.

## Validation

- [x] Run `pnpm exec tsx proofs/070/smoke.ts`.

...
All implementation notes, fixtures, and smoke tests for this proof live under `proofs/080/`. Run with `pnpm exec tsx proofs/080/smoke.ts`; do not add a root `package.json` script.
...
`pnpm exec tsx proofs/080/smoke.ts` proves: build one bundle from guest capture, V8 decoder, thread classifier, and resource emitter artifacts.

## Validation

- [x] Run `pnpm exec tsx proofs/080/smoke.ts`.

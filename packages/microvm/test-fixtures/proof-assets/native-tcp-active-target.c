...
#define DATA_FILE "/tmp/machinen-native-tcp-active-target-data.txt"

static char file_read_buffer[4] __attribute__((used));

static void die(const char *what) {
  fprintf(stderr, "machinen-native-tcp-active-target: %s: %s\n", what, strerror(errno));
  _exit(1);
}
...
int main(void) {
  ...
  int yes = 1;
  if (setsockopt(listener, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) != 0) die("setsockopt");
  ...
  if (getsockname(listener, (struct sockaddr *)&addr, &addr_len) != 0) die("getsockname");
  if (listen(listener, 1) != 0) die("listen");
  int client = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if (client < 0) die("client socket");
  if (connect(client, (struct sockaddr *)&addr, sizeof(addr)) != 0) die("connect");
  int peer = accept4(listener, NULL, NULL, SOCK_CLOEXEC);
  if (peer < 0) die("accept4");
  close(listener);
  const char payload[] = "PING";
  if (write(peer, payload, sizeof(payload) - 1) != (ssize_t)(sizeof(payload) - 1)) die("seed write");
  if (dup2(client, 56) < 0) die("dup2 client");
  if (dup2(peer, 57) < 0) die("dup2 peer");
  if (client != 56) close(client);
  if (peer != 57) close(peer);
  fprintf(stderr, "MACHINEN_NATIVE_TCP_ACTIVE_READY fd=56 brokerFd=57\n");
  fflush(stderr);

...
#define TARGET_PING_SOCKET_FD 59
#define DATA_FILE "/tmp/machinen-native-ping-socket-target-data.txt"
#define ICMP_IDENTIFIER 0x4d50u
#define ICMP_SEQUENCE 2u

static char file_read_buffer[4] __attribute__((used));

static void die(const char *what) {
  fprintf(stderr, "machinen-native-ping-socket-target: %s: %s\n", what, strerror(errno));
  _exit(1);
}
...
static void drain_ping_socket_queue(int fd) {
  for (int attempt = 0; attempt < 8; attempt++) {
    struct pollfd pfd = {.fd = fd, .events = POLLIN};
    int ready = poll(&pfd, 1, 0);
    if (ready < 0) die("ping socket drain poll");
    if (ready == 0) return;
    uint8_t buffer[256];
    ssize_t got = recv(fd, buffer, sizeof(buffer), MSG_DONTWAIT);
    if (got < 0) {
      if (errno == EAGAIN || errno == EWOULDBLOCK) return;
      die("ping socket drain recv");
    }
  }
  fprintf(stderr, "machinen-native-ping-socket-target: receive queue did not drain\n");
  _exit(1);
}
...
static void verify_loopback_echo(int fd) {
  ...
  if (sendto(fd, packet, sizeof(packet), 0, (struct sockaddr *)&addr, sizeof(addr)) != (ssize_t)sizeof(packet)) {
    die("ping socket sendto");
  }
  for (int attempt = 0; attempt < 8; attempt++) {
    struct pollfd pfd = {.fd = fd, .events = POLLIN};
    int ready = poll(&pfd, 1, 250);
    if (ready < 0) die("ping socket poll");
    if (ready == 0) continue;
    uint8_t buffer[256];
    ssize_t got = recv(fd, buffer, sizeof(buffer), 0);
    if (got < 0) die("ping socket recv");
    if (got < (ssize_t)sizeof(struct icmphdr)) continue;
    ...
    if (reply->type == ICMP_ECHOREPLY && reply->code == 0 &&
      ...
      drain_ping_socket_queue(fd);
      return;
    }
  }
  fprintf(stderr, "machinen-native-ping-socket-target: loopback echo timed out\n");
  _exit(1);
}
...
int main(void) {
  ...
  if (fd < 0) die("ping socket");
  struct sockaddr_in local = {0};
  local.sin_family = AF_INET;
  local.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
  local.sin_port = htons(ICMP_IDENTIFIER);
  if (bind(fd, (struct sockaddr *)&local, sizeof(local)) != 0) die("ping socket bind");
  verify_loopback_echo(fd);
  if (dup2(fd, TARGET_PING_SOCKET_FD) < 0) die("dup2 ping socket");
  if (fd != TARGET_PING_SOCKET_FD) close(fd);
  fprintf(stderr, "MACHINEN_NATIVE_PING_SOCKET_READY fd=%d id=%u seq=%u uid=%u gid=%u\n", TARGET_PING_SOCKET_FD, ICMP_IDENTIFIER, ICMP_SEQUENCE, (unsigned)getuid(), (unsigned)getgid());
  fflush(stderr);

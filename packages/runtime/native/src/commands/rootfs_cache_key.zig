...
pub fn run(allocator: std.mem.Allocator, io: std.Io) !protocol.Exit {
    ...
    const result = runtime_helper.rootfs.rootfsCacheKey(
        allocator,
        io,
        request.tar,
    ) catch |err| {
        switch (err) {
            error.PathNotFound => try protocol.writeError(
                ...
                "ensureRootfsImage: tarball not found",
            ),
    ...
    try protocol.writeJson(allocator, io, .{
        ...
        .command = name,
        .data = .{ .sha = result.sha[0..], .source = source },
    });
    return .ok;
}
...
fn writeRequestError(io: std.Io, err: RequestError) !void {
    ...
    switch (err) {
        ...
        error.MissingTar => try protocol.writeError(
            io,
            "INVALID_REQUEST",
            "rootfs-cache-key request data must include a string tar field",
        ),
        error.InvalidTar => try protocol.writeError(
            io,
            "INVALID_REQUEST",
            "rootfs-cache-key tar field must be a string",
        ),

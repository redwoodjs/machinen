...
pub fn run(allocator: std.mem.Allocator, io: std.Io) !protocol.Exit {
    ...
    const result = runRequest(allocator, io, request) catch |err| {
        switch (err) {
            error.PathNotFound => try protocol.writeError(io, "MKINITRAMFS_INPUT_NOT_FOUND", "mkinitramfs input path does not exist"),
            error.PathNotDirectory => try protocol.writeError(io, "MKINITRAMFS_INPUT_INVALID", "mkinitramfs input path is not a directory"),
            error.InitMissing => try protocol.writeError(io, "MKINITRAMFS_INIT_MISSING", "mkinitramfs: /init binary not readable"),
            error.WorkspaceTooLarge => try protocol.writeError(io, "MKINITRAMFS_WORKSPACE_TOO_LARGE", "workspace exceeds the configured maxMb"),
    ...
    const out = try std.fmt.allocPrint(
        allocator,
        "{{\"ok\":true,\"protocolVersion\":1,\"command\":\"mkinitramfs\",\"data\":{{\"out\":\"{s}\",\"bytes\":{d},\"workspaceBytes\":{d}}}}}\n",
        .{ request.out, result.bytes, result.workspace_bytes },
    );
...
fn runRequest(allocator: std.mem.Allocator, io: std.Io, request: Request) runtime_helper.mkinitramfs.Error!runtime_helper.mkinitramfs.Result {
    ...
    if (std.mem.eql(u8, request.mode, "tiny")) {
        return runtime_helper.mkinitramfs.packTiny(allocator, io, .{ .out = request.out, .final = final });
    }
    if (std.mem.eql(u8, request.mode, "rootfs")) {
        return runtime_helper.mkinitramfs.packRootfs(allocator, io, .{ .rootfs = request.rootfs orelse return error.PathNotFound, .out = request.out, .excludes = request.excludes, .final = final });
    }
    if (std.mem.eql(u8, request.mode, "workspace")) {
        return runtime_helper.mkinitramfs.packWorkspace(allocator, io, .{ .workspace = request.workspace orelse return error.PathNotFound, .out = request.out, .mountpoint = request.mountpoint orelse "workspace", .excludes = request.excludes, .max_mb = request.max_mb orelse 500 });
    }
    if (std.mem.eql(u8, request.mode, "minimal")) {
        return runtime_helper.mkinitramfs.packMinimal(allocator, io, .{ .out = request.out, .final = final });
    }
    unreachable;
}
...
fn parseRequest(allocator: std.mem.Allocator, io: std.Io) RequestError!Request {
    ...
    try protocol.rejectUnknownFields(object, &.{ "mode", "out", "rootfs", "workspace", "mountpoint", "excludes", "maxMb", "initPath", "config", "injectInit", "allowMissingInit", "execAgentPath", "mountGuest" });

    const mode = try requiredString(object, "mode", error.MissingMode, error.InvalidMode);
    if (!std.mem.eql(u8, mode, "tiny") and !std.mem.eql(u8, mode, "rootfs") and !std.mem.eql(u8, mode, "workspace") and !std.mem.eql(u8, mode, "minimal")) return error.InvalidMode;
    var request: Request = .{ .mode = mode, .out = try requiredString(object, "out", error.MissingOut, error.InvalidOut) };
    ...
    request.exec_agent_path = try optionalString(object, "execAgentPath", error.InvalidExecAgentPath);
    request.mount_guest = try optionalString(object, "mountGuest", error.InvalidMountGuest);
    request.excludes = try optionalStringArray(allocator, object, "excludes", error.InvalidExcludes);
    request.max_mb = try optionalUsize(object, "maxMb", error.InvalidMaxMb);
    request.inject_init = try optionalBool(object, "injectInit", error.InvalidInjectInit) orelse true;
    request.allow_missing_init = try optionalBool(object, "allowMissingInit", error.InvalidAllowMissingInit) orelse false;

    if (std.mem.eql(u8, mode, "rootfs") and request.rootfs == null) return error.MissingRootfs;
    if (std.mem.eql(u8, mode, "workspace") and request.workspace == null) return error.MissingWorkspace;
    return request;
}
...
fn optionalUsize(object: std.json.ObjectMap, field: []const u8, invalid: RequestError) RequestError!?usize {
    const value = object.get(field) orelse return null;
    if (value == .null) return null;
...
fn writeRequestError(io: std.Io, err: RequestError) !void {
    switch (err) {
        ...
        error.MissingMode => try protocol.writeError(io, "INVALID_REQUEST", "mkinitramfs request data must include a string mode field"),
        error.InvalidMode => try protocol.writeError(io, "INVALID_REQUEST", "mkinitramfs mode must be one of tiny, rootfs, workspace, or minimal"),
        error.MissingOut => try protocol.writeError(io, "INVALID_REQUEST", "mkinitramfs request data must include a string out field"),
        error.MissingRootfs => try protocol.writeError(io, "INVALID_REQUEST", "mkinitramfs rootfs mode requires a rootfs field"),
        error.MissingWorkspace => try protocol.writeError(io, "INVALID_REQUEST", "mkinitramfs workspace mode requires a workspace field"),

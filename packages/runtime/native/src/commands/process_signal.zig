...
pub fn run(allocator: std.mem.Allocator, io: std.Io) !protocol.Exit {
    ...
    try protocol.stdout(io, "\"command\":\"process-signal\",\"data\":{");
...
fn parseRequest(allocator: std.mem.Allocator, io: std.Io) RequestError!Request {
    ...
    if (pid_value != .integer or
        pid_value.integer <= 0 or
        pid_value.integer > std.math.maxInt(u32))
    {
    ...
    const signal = runtime_helper.lifecycle.parseSignal(signal_value.string) orelse
...
fn writeSignalError(io: std.Io, err: runtime_helper.lifecycle.ProcessSignalError) !void {
    assert(@sizeOf(runtime_helper.lifecycle.ProcessSignalError) > 0);

    switch (err) {
        error.InvalidPid => try protocol.writeError(
            io,
            "INVALID_REQUEST",
            "pid must be a positive process id",
        ),
        error.InvalidSignal => try protocol.writeError(
            io,
            "INVALID_REQUEST",
            "signal must be SIGTERM, SIGKILL, or 0",
        ),
        error.PermissionDenied => try protocol.writeError(
            io,
            "PROCESS_SIGNAL_DENIED",
            "permission denied while signalling process",
        ),
        error.Unexpected => try protocol.writeError(
            io,
            "PROCESS_SIGNAL_FAILED",
            "unexpected error while signalling process",
        ),
    }
}
...
fn writeRequestError(io: std.Io, err: RequestError) !void {
    ...
    switch (err) {
        ...
        error.MissingPid => try protocol.writeError(
            io,
            "INVALID_REQUEST",
            "pid is required",
        ),
        ...
        error.MissingSignal => try protocol.writeError(
            io,
            "INVALID_REQUEST",
            "signal is required",
        ),

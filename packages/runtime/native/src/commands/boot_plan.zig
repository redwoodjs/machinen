...
const ParsedRequest = struct {
    ...
    restore_image_explicit_path: ?[]const u8,
    restore_image_explicit_exists: ?bool,
    restore_image_meta_source_path: ?[]const u8,
    restore_image_meta_source_exists: ?bool,
    ...
    scratch_temp_dir: ?[]const u8,
    scratch_temp_pid_text: ?[]const u8,
    scratch_temp_nonce: ?[]const u8,
    ...
    root_disk_temp_kind: boot_plan.RootDiskTempPathKind,
    root_disk_temp_dir: ?[]const u8,
    root_disk_temp_pid_text: ?[]const u8,
    root_disk_temp_nonce: ?[]const u8,
    ...
    mount_disk_temp_kind: boot_plan.MountDiskTempPathKind,
    mount_disk_temp_dir: ?[]const u8,
    mount_disk_temp_pid_text: ?[]const u8,
    mount_disk_temp_nonce: ?[]const u8,
...
pub fn run(allocator: std.mem.Allocator, io: std.Io) !protocol.Exit {
    ...
    const restore_image = boot_plan.planRestoreImage(.{
    ...
    const scratch_temp_pid = if (parsed.scratch_temp_pid_text) |text|
        parseUnsigned(text) catch {
        ...
        null;
    const scratch_temp_path = boot_plan.planScratchTempPath(arena, .{
        .kind = parsed.scratch_temp_kind,
        .tmp_dir = parsed.scratch_temp_dir,
        .pid = scratch_temp_pid,
    ...
    const root_disk_temp_pid = if (parsed.root_disk_temp_pid_text) |text|
        parseUnsigned(text) catch {
        ...
        null;
    const root_disk_temp_path = boot_plan.planRootDiskTempPath(arena, .{
        .kind = parsed.root_disk_temp_kind,
        .tmp_dir = parsed.root_disk_temp_dir,
        .pid = root_disk_temp_pid,
    ...
    const mount_disk_temp_pid = if (parsed.mount_disk_temp_pid_text) |text|
        parseUnsigned(text) catch {
        ...
        null;
    const mount_disk_temp_path = boot_plan.planMountDiskTempPath(arena, .{
        .kind = parsed.mount_disk_temp_kind,
        .tmp_dir = parsed.mount_disk_temp_dir,
        .pid = mount_disk_temp_pid,
...
fn writePlan(
    ...
    try protocol.stdout(io, ",\"scratchTempPath\":");
    try writeNullableJsonString(io, scratch_temp_path.path);
    ...
    try protocol.stdout(io, "}");
    try protocol.stdout(io, ",\"rootDiskTempPath\":");
    try writeNullableJsonString(io, root_disk_temp_path.path);
    ...
    try protocol.stdout(io, ",\"mountDiskTempPath\":");
    try writeNullableJsonString(io, mount_disk_temp_path.path);
    ...
    try protocol.stdout(io, "]");
    try protocol.stdout(io, ",\"restoreImage\":{");
    try protocol.stdout(io, "\"path\":");
    try writeNullableJsonString(io, restore_image.path);
    try protocol.stdout(io, ",\"error\":");
    try writeNullableJsonString(io, restore_image.error_kind);
    try protocol.stdout(io, "}");
...
fn parseRequest(allocator: std.mem.Allocator, io: std.Io) RequestError!ParsedRequest {
    ...
    return .{
        ...
        .restore_image_explicit_path = try optionalStringDefaultNull(object, "restoreImageExplicitPath", error.InvalidRestoreImageExplicitPath),
        .restore_image_explicit_exists = try optionalBoolDefaultNull(object, "restoreImageExplicitExists", error.InvalidRestoreImageExplicitExists),
        .restore_image_meta_source_path = try optionalStringDefaultNull(object, "restoreImageMetaSourcePath", error.InvalidRestoreImageMetaSourcePath),
        .restore_image_meta_source_exists = try optionalBoolDefaultNull(object, "restoreImageMetaSourceExists", error.InvalidRestoreImageMetaSourceExists),
        ...
        .scratch_temp_dir = try optionalStringDefaultNull(object, "scratchTempDir", error.InvalidScratchTempDir),
        .scratch_temp_pid_text = try optionalStringDefaultNull(object, "scratchTempPid", error.InvalidScratchTempPid),
        .scratch_temp_nonce = try optionalStringDefaultNull(object, "scratchTempNonce", error.InvalidScratchTempNonce),
        ...
        .root_disk_temp_kind = try optionalRootDiskTempKind(object),
        .root_disk_temp_dir = try optionalStringDefaultNull(object, "rootDiskTempDir", error.InvalidRootDiskTempDir),
        .root_disk_temp_pid_text = try optionalStringDefaultNull(object, "rootDiskTempPid", error.InvalidRootDiskTempPid),
        .root_disk_temp_nonce = try optionalStringDefaultNull(object, "rootDiskTempNonce", error.InvalidRootDiskTempNonce),
        ...
        .mount_disk_temp_kind = try optionalMountDiskTempKind(object),
        .mount_disk_temp_dir = try optionalStringDefaultNull(object, "mountDiskTempDir", error.InvalidMountDiskTempDir),
        .mount_disk_temp_pid_text = try optionalStringDefaultNull(object, "mountDiskTempPid", error.InvalidMountDiskTempPid),
        .mount_disk_temp_nonce = try optionalStringDefaultNull(object, "mountDiskTempNonce", error.InvalidMountDiskTempNonce),
...
fn writeRequestError(io: std.Io, err: RequestError) !void {
    switch (err) {
        ...
        error.InvalidRootDiskTempKind => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan rootDisk temp kind must be restore or cached"),
        error.InvalidRootDiskTempDir, error.InvalidRootDiskTempNonce => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan rootDisk temp fields must be strings"),
        error.InvalidRootDiskTempPid => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan rootDisk temp pid must be a decimal integer"),
        ...
        error.InvalidMountDiskTempKind => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan mountDisk temp kind must be restore-upper"),
        error.InvalidMountDiskTempDir, error.InvalidMountDiskTempNonce => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan mountDisk temp fields must be strings"),
        error.InvalidMountDiskTempPid => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan mountDisk temp pid must be a decimal integer"),
        error.InvalidScratchOptionFalse => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan scratch option false flag must be a boolean"),
        error.InvalidScratchOptionPath => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan scratch option path must be a string"),
        error.InvalidScratchTempKind => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan scratch temp kind must be restore or auto"),
        error.InvalidScratchTempDir, error.InvalidScratchTempNonce => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan scratch temp fields must be strings"),
        error.InvalidScratchTempPid => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan scratch temp pid must be a decimal integer"),
        error.InvalidProvisionDtbExplicit => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan provision dtb explicit flag must be a boolean"),
        error.InvalidProvisionCliCacheHome, error.InvalidProvisionCliCacheVersion => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan provision cli cache fields must be strings"),
        error.InvalidProvisionAssetExplicitPath, error.InvalidProvisionAssetAssetsDirPath, error.InvalidProvisionAssetCachePath => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan provision asset lookup paths must be strings"),
        error.InvalidProvisionAssetExplicitExists, error.InvalidProvisionAssetAssetsDirExists, error.InvalidProvisionAssetCacheExists => try protocol.writeError(io, "INVALID_REQUEST", "boot-plan provision asset lookup exists flags must be booleans"),

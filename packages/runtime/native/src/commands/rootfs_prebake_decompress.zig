...
pub fn run(allocator: std.mem.Allocator, io: std.Io) !protocol.Exit {
    ...
    const out = if (result.ok and result.sha256 != null)
        try std.fmt.allocPrint(
            allocator,
            "{{\"ok\":true,\"protocolVersion\":1,\"command\":\"rootfs-prebake-decompress\",\"data\":{{\"ok\":true,\"sha256\":\"{s}\"}}}}\n",
            .{&result.sha256.?},
        )
    else
        try allocator.dupe(u8, "{\"ok\":true,\"protocolVersion\":1,\"command\":\"rootfs-prebake-decompress\",\"data\":{\"ok\":false}}\n");

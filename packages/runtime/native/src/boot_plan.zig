...
pub fn planProvisionCliCacheBaseDir(allocator: std.mem.Allocator, input: ProvisionCliCacheInput) !ProvisionCliCachePlan {
...
pub fn planBundleEnv(allocator: std.mem.Allocator, input: BundleEnvInput) ![]EnvPair {
    return planVmmEnv(allocator, .{ .base = input.image_env, .overrides = input.guest_env });
}
...
test "planProvisionBoot builds provision boot inputs and vmm env" {
    ...
    try std.testing.expectEqual(@as(usize, 2), plan.vmm_env.len);
    try std.testing.expectEqualStrings("MACHINEN_BOOT_TEST", plan.vmm_env[0].key);

...
export type { BootOptions } from "./boot-options.ts";
import type { BootOptions } from "./boot-options.ts";
...
export async function boot(opts: BootOptions = {}): Promise<VmHandle> {
  ...
  const spawned = await spawnBootVmmOrRollback({ opts, plan, resources, phases, bootT0 });
  const child = spawned.child;
  const registry = await registerSpawnedBoot({ opts, plan, resources, spawned, bootT0 });
  const {
    childPid,
    vmName,
    sourceImageAbs,
  ...
  const outputRelays = installBootOutputRelays({
    child,
    phases,
    onLog: opts.onLog,
  ...
  const handle = createBootVmHandle({
    ...
    vmName,
    timeoutMs,
    outputCollector: outputRelays.outputCollector,
    errorCollector: outputRelays.errorCollector,
    vsockUdsPath,
    onLog: opts.onLog,
    statsFilePath,
    memoryCeilingMib,
  ...
  maybeSetGuestHostname({ handle, vsockUdsPath, env });
  await maybeGateDetachedReadiness({
    opts,
    bootLogPath,
    child,
    timeoutMs,
    detachedBootChunks: outputRelays.detachedBootChunks,
    handle,
    plan,
    resources,
    spawned,
...
function logBootEntry(opts: BootOptions): void {
  debug(
...
async function spawnBootVmmOrRollback(args: SpawnBootArgs): Promise<SpawnedBootVmm> {
  try {
    return await spawnBootVmm(args);
  } catch (err) {
    rollbackPreSpawn({
...
function installBootOutputRelays(args: {
  ...
  const outputCollector = collect(args.child.stdout);
  const errorCollector = collect(args.child.stderr);
  if (args.onLog) {
    args.child.stderr.on("data", (chunk: Buffer) => {
      args.onLog?.({ source: "guest-console", chunk });
    });
  }
  ...
  installVmstateTimingRelay(args.child);
  installFlushPhases(args.child, args.phases, args.onLog);
  ...
  if (args.detached) {
    installDetachedBootCapture(args.child, detachedBootChunks);
  }
  return { outputCollector, errorCollector, detachedBootChunks };
}

function maybeSetGuestHostname(args: {
  handle: VmHandle;
  ...
  if (args.vsockUdsPath && args.env.MACHINEN_SKIP_GUEST_HOSTNAME !== "1") {
    void setGuestHostname(args.handle, buildGuestHostname(args.handle.pid, args.handle.name));
  }
}

async function maybeGateDetachedReadiness(args: {
  opts: BootOptions;
  ...
  registry: BootRegistryState;
  bootT0: number;
}): Promise<void> {
  if (!args.opts.detached || !args.bootLogPath) {
    return;
  }
  try {
    await gateOnDetachedReadiness({
      ...
      detachedBootChunks: args.detachedBootChunks,
      handle: args.handle,
    });
  } catch (err) {
    await cleanupSpawnedBootFailure({
      plan: args.plan,
...
async function spawnBootVmm(args: SpawnBootArgs): Promise<SpawnedBootVmm> {
  ...
  let mountDiskFds: { lowerFd: number; upperFd: number } | undefined;
  try {
    mountDiskFds = maybeOpenMountDiskFds(args.resources.mountDiskPaths, args.plan.env, stdio);
...
async function registerSpawnedBoot(args: {
  opts: BootOptions;
  plan: BootPlan;
  resources: BootResources;
  spawned: SpawnedBootVmm;
  bootT0: number;
}): Promise<BootRegistryState> {
  const state = buildBootRegistryState(args.opts, args.resources, args.spawned);
  let registered = false;
  installBootExitCleanup(args, state, () => registered);
  try {
    claimBootNameIfNeeded(state);
    registered = writeBootRegistryIfPossible(args, state);
    return state;
  } catch (err) {
    await cleanupSpawnedBootFailure({
      plan: args.plan,
...
function claimBootNameIfNeeded(state: BootRegistryState): void {
  if (state.vmName && state.childPid > 0) {
    claimNameOrThrow(state.vmName, state.childPid);
  }
}
...
function claimNameOrThrow(vmName: string, childPid: number): void {
...
function installVmExitCleanup(state: ExitCleanupState): void {
  state.child.once("exit", (code, signal) => {
    ...
    cleanupBootArtifacts(state);
  });
}

function cleanupBootArtifacts(state: ExitCleanupState): void {
  for (const file of [state.perBootRootDisk, state.perBootSnapDisk, state.perBootMountUpper]) {
    if (file) {
      try {
        unlinkSync(file);
...
async function cleanupSpawnedBootFailure(args: {
  plan: BootPlan;
  resources: BootResources;
  spawned: SpawnedBootVmm;
  state: BootRegistryState;
  ...
  cleanupBootArtifacts({
    child: args.spawned.child,
    childPid: args.state.childPid,
    bootT0: args.bootT0,
    perBootRootDisk: args.resources.perBootRootDisk,
    perBootSnapDisk: args.plan.perBootSnapDisk,
    perBootMountUpper: args.spawned.perBootMountUpper,
  ...
  await terminateChildForBootFailure(args.spawned.child);
}

async function terminateChildForBootFailure(child: ChildProcessWithoutNullStreams): Promise<void> {
  ...
  if ((await Promise.race([exited, timeout])) === "exit") {
    return;
  }
  try {
    child.kill("SIGKILL");
  } catch {}
  const killed = once(child, "exit").then(() => undefined);
  const killTimeout = new Promise<void>((resolve) => {
    setTimeout(resolve, 500).unref();
  });
  await Promise.race([killed, killTimeout]);
}

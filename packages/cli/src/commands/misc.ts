...
_machinen_completion() {
  ...
  case "\${words[1]}" in
    ...
    run)
      ...
      ;;
    terminal)
      COMPREPLY=( $(compgen -W "new list inspect attach send signal stop delete reconcile gc --database" -- "\${cur}") )
      return
      ;;
    gc)
...
_machinen() {
  ...
  case "\${words[2]}" in
    ...
    run)
      ...
      ;;
    terminal)
      _describe 'operation' '(new list inspect attach send signal stop delete reconcile gc --database)'
      return
      ;;
    gc)
...
for bin in machinen mn
  ...
  complete -c $bin -f -n "__fish_seen_subcommand_from terminal" -a "new list inspect attach send signal stop delete reconcile gc --database"

...
Claude Code computes the slug directory by replacing `/` with `-` in the absolute cwd path. For example, `/Users/justin/rw/sdk` becomes `-Users-justin-rw-sdk`. This is a direct string replacement, not a hashing scheme.

...
These states are refused fail-closed by the portable VM product plan or by the broader compatibility matrix until separately proven. Rows `063` through `312` add 108 supported semantic Memory IR rows and 142 additional fail-closed rows. Fifty-six formerly refused rows are now verified only through retained semantic Node Resource IR product proof (timer/scheduler specs, drained streams/pipelines, reopenable file/stream specs, app registries, reconnectable outbound-client configs, and idle/drained observability specs). The remaining unsafe rows are `verified-refusal`: they have retained fail-closed proof and still are not supported as raw/live/native continuation.

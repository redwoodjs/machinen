...
Exact live product path metadata is now treated as proof metadata, not as a product move continuation claim by itself. A descriptor-harness row such as `generic-service-nginx-static-parity` or an adjacent generic support row may prove that a synthetic shape can run under the generic loader, but it is not enough to preempt an explicit product loader or claim `machinen move` continuation. Product move continuation requires modeled live execution/resource state continuation; target-native reexec, listener restart, static-root reconstruction, file cursor reopen, pipe byte replay, and refusal evidence are not continuation. The current product move continuation allowlist is empty.

The classification ledger at `docs/snapshot/move-continuation-boundary-classification.json` records every current move proof/product row as exactly one of `continuation`, `resource-reconstruction`, `reexec`, or `refusal`. At the time of this boundary split the ledger covers 243 rows: 75 resource-reconstruction rows, 49 reexec rows, 118 refusal rows, and one continuation row. The one continuation row, `generic-same-arch-modeled-continuation`, remains proof-only until cross-architecture target-native live-state reconstruction and equivalent refusals are proven.

### Former productization rows relabeled outside product move continuation

The phase-1, wave-2, and static HTTP productization artifacts remain retained evidence, but they are no longer product move continuation routes:

| Historical row                                       | Current label           | Why it is not product move continuation                                                                                               | Retained evidence remains useful for                                                                                                                                                |
| ---------------------------------------------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `generic-stdio-pipe-product-marker`                  | resource-reconstruction | captured finite pipe bytes are replayed into a target-native consumer instead of continuing live pipe/process state                   | modeled stdio/pipe proof evidence with `generic-finite-pipe-buffer-replay` and refusal proof `generic-pipe-stdio-refusals`                                                          |
| `reader-cat-live-generic-primary-marker`             | resource-reconstruction | the target reopens a readonly file cursor and reproduces output rather than continuing a live process                                 | file identity/cursor proof evidence plus stale/deleted/writable-file refusals                                                                                                       |
| `grep-live-generic-primary-marker`                   | resource-reconstruction | the target reopens a readonly file cursor and reexecs grep with captured argv rather than continuing grep execution                   | grep/file cursor proof evidence plus stale/deleted/writable-file and pipe/stdio refusals                                                                                            |
| `unix-pathname-listener-live-generic-primary-marker` | reexec                  | the target recreates an idle pathname listener socket instead of continuing an existing socket/session                                | pathname listener reconstruction evidence and connected/abstract/datagram/fd-passing refusal boundaries                                                                             |
| `busybox-nc-listener-live-generic-primary-marker`    | reexec                  | the target restarts an idle BusyBox nc listener with modeled stdio/log behavior instead of continuing a socket/session                | idle listener restart proof evidence and active-client/package/preflight refusals: `unsafe-busybox-nc-refusal`, `unsafe-nc-active-refusal`, and `generic-loader-preflight-refusals` |
| `socat-file-responder-live-generic-primary-marker`   | reexec                  | the target restarts socat against a stable response file instead of continuing socat runtime/socket state                             | exact socat file-responder restart evidence and active-client/file-drift/package/preflight refusals: `unsafe-socat-file-responder-refusal` and `generic-loader-preflight-refusals`  |
| `node-static-http-live-generic-primary-marker`       | reexec                  | target-native Node reexec with static-root identity does not continue V8 heap, JS stack, event loop phase, libuv handles, or sessions | static-root identity, health/body proof, and Node active-client/timer/worker/native-addon refusals                                                                                  |
| `go-static-http-live-generic-primary-marker`         | reexec                  | target-native Go reexec with static-root identity does not continue arbitrary Go runtime or socket/session state                      | static-root identity, health/body proof, and extra-socket/tree-drift refusals                                                                                                       |
| `rust-static-http-live-generic-primary-marker`       | reexec                  | target-native Rust reexec with static-root identity does not continue arbitrary native process or socket/session state                | static-root identity, health/body proof, and loader/tree-drift refusals                                                                                                             |
| `busybox-httpd-live-generic-primary-marker`          | reexec                  | exact BusyBox httpd restart with static-root identity does not continue process/socket/session state                                  | static-root identity, body proof, and active-client/root/bind/CGI/preflight/tree-drift refusals                                                                                     |

Retained validation artifacts remain documented for regression evidence: `scripts/smoke/move-envelope-productization-phase1-plan.json`, `scripts/smoke/move-envelope-productization-wave2-plan.json`, `scripts/smoke/move-envelope-productization-wave2-validation-profile.json`, `scripts/smoke/move-envelope-productization-static-http-tree-identity-plan.json`, `/tmp/machinen-productization-phase1-retained-coverage`, `/tmp/machinen-productization-wave2-retained-coverage`, and `/tmp/machinen-productization-static-http-tree-identity-retained-coverage`. They must be described as reconstruction/reexec/refusal proof evidence unless a later continuation-only contract proves modeled live-state continuation. Static HTTP retained refusal proof names are `node-active-refusal`, `node-timer-refusal`, `node-worker-refusal`, `native-dlopen-refusal`, `go-extra-socket-refusal`, `generic-loader-preflight-refusals`, `unsafe-busybox-httpd-refusal`, `python-http-active-refusal`, and `static-http-tree-identity-refusals`.

Static HTTP tree identity does not add arbitrary process restore, any-binary movement, broad daemon/database/service migration, active session migration, source-fd teleportation, source-ISA emulation, metadata-only success, runtime-profile shortcut, proof-only descriptor support, proof-image runtime dependency, or product move continuation. In short: no runtime-profile shortcut.

### Future move work: continuation only

Hard product rule: `machinen move` must refuse target-native reexec, restart, and resource reconstruction. Continuation is not banned, but it must be modeled live-state continuation before any target process starts. Acceptable future work starts from captured source machine/process/resource state and reconstructs or translates that state target-natively so execution can continue from the modeled state, not from an app restart, static-root reexec, output replay, or descriptor-only equivalence.

Allowed future continuation ladders include:

1. modeled fd/resource state continuation with explicit source offsets, flags, readiness, blocking, and target validation;
2. append-only log continuation only after concurrent-writer, EOF, identity, and event-delivery semantics are modeled;
3. timer/event-loop resource continuation only after remaining time, clock basis, waiters, and missed-event policy are captured and target-native;
4. finite socket/request/session boundaries only after queued bytes, accept/connect state, peer identity, credentials, socket options, and active-session semantics are modeled;
5. Node continuation only after the model covers the relevant V8 heap state, JS stack/event-loop phase, libuv handles, workers, native addons, timers, sockets, and in-memory app/session state, with fail-closed refusals for every unmodeled piece.

The first concrete lab primitive contract on this ladder is [same-arch stopped continuation](./same-arch-stopped-continuation-primitive.md). It is contract-only/not-promoted and same-architecture only, so it is not product move support. The current narrow product ladder is [cross-ISA CLI next binaries](./cross-arch-cli-next-binaries.md): `cat` may continue captured input/output cursor state; regular-file `dd` may continue captured offsets, partial block state, and counters; `wc -l` may continue captured byte offset and line count; integer `seq` may continue captured current/next value and output cursor; and fixed-string `grep -F` may continue captured input cursor, parser state, matcher state, match count, and output cursor. Broad grep, broad wc, unsafe dd, terminal/session shapes, pipes, sockets, services, databases, shells, same-arch product success, argv restart, target execve from argv, output replay, descriptor-only success, source-ISA emulation, source-fd teleportation, and metadata-only success remain refused without explicit semantic models.

Not acceptable as product move continuation: target-native reexec, restart from argv/files, static HTTP reconstruction, static-root tree identity alone, metadata-only equivalence, app-exported state, checkpoint hooks, selected-state descriptors, sidecar replay, source-ISA emulation, source-fd teleportation, runtime-profile shortcuts, broad daemon/database/service migration, arbitrary process restore, or active session migration without an explicit model.
...
| Static HTTP full tree digest identity     | resolved only for the four exact static HTTP tree-identity rows documented above; every other static/service shape remains deferred            | no generic product support without source/target static-root digest evidence and tree-drift refusals; historical blocker phrase: Node static full tree digest identity / no Node static HTTP generic product support in phase 1 |
...
These rows intentionally model exact marker-proven static HTTP/listener/file-cursor reexec, not source-fd continuation. The static HTTP tree-identity rows require source/target static-root digest evidence, `staticRootTreeIdentity`, and tree-drift refusals before product routing; BusyBox httpd product rows require stable root identity and an explicit loopback bind.

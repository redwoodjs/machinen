...
The first concrete primitive contract on this ladder is [same-arch stopped continuation](./same-arch-stopped-continuation-primitive.md). It is currently contract-only/not-promoted and same-architecture only. Its capture classifier accepts only a stopped single-thread safe point with captured instruction pointer, stack pointer, registers, private writable mappings, stack bytes, text identity, inert stdio, and empty/default fd/socket/timer/signal/session state. Its target materialization guard requires verified target text identity, materialized memory, installed registers, clean target preflight, targetPid, and a captured-state-dependent marker that differs from fresh restart output before reporting `resumedFromCapturedState=true`.

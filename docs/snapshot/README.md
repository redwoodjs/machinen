...
The generic direction is resource-driven support: move a selected binary only when its observed resource graph is fully understood, and refuse exact unsupported resource classes. This is exact target-native resource graph support only, not arbitrary process restore or any-binary movement. Phase 1 product generic routing is limited to `generic-stdio-pipe-product-marker`: modeled finite stdio pipe replay with `migration.productPath.kind="exact-live-capture"`, support proof `generic-finite-pipe-buffer-replay`, refusal proof `generic-pipe-stdio-refusals`, and `refusalClasses=[]`. It also makes no broad daemon/database migration, no active session migration, no source-fd teleportation, no source-ISA emulation, and no metadata-only success claim. See [Generic resource graph move envelope](./generic-resource-graph-move.md) for the contract and inventory, [Generic resource graph migration wave 1](./generic-resource-graph-migration-wave1.md) for selected bespoke-to-generic-equivalent migration boundaries, [Generic resource graph graduation frontier](./generic-resource-graph-frontier.md) for unsupported resource-class follow-up contracts, [Generic pipes/stdio graduation](./generic-pipes-stdio-graduation.md) for the pipe/stdio contract, and the generic move envelope's regular-file cursor rows for read-only fd cursors plus exact EOF-only append-log fd continuation. Wave 2 keeps refusal/evidence baselines for unsafe append variants, Unix socket subtypes, and anon-inode subtypes; it does not imply arbitrary writable-file or any-binary movement.

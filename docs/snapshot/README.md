...
The generic direction is resource-driven support: move any binary whose observed resource graph is fully understood, and refuse exact unsupported resource classes. See [Generic resource graph move envelope](./generic-resource-graph-move.md) for the contract and inventory, and [Generic resource graph graduation frontier](./generic-resource-graph-frontier.md) for the unsupported resource-class follow-up contracts.

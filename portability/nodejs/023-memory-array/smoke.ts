...
const recovery = recoverNodeProperLevel5V8ObjectStateEvidence(
  [{ bytes: fragment([2, 1, 2]), bytesPath: "memory/array.bin" }],
  { anchor, expectedTotal: 2, expectedHistory: [1, 2] },
);
const materialized = [...(recovery.candidates[0]?.history ?? [])];
const accepted = recovery.accepted && JSON.stringify(materialized) === JSON.stringify([1, 2]);
const report = {
  ...
  claimBoundary: {
    claims: ["selected packed V8 Smi array state is decoded and semantically materialized"],

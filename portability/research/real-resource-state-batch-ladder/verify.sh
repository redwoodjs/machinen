...
AMD64_HOST=${REAL_RESOURCE_BATCH_AMD64_HOST:-root@192.168.0.8}
ARM64_HOST=${REAL_RESOURCE_BATCH_ARM64_HOST:-friend@100.126.46.90}
RUN_ID=${REAL_RESOURCE_BATCH_RUN_ID:-$(date -u +%Y%m%dT%H%M%SZ)-$$}
AMD64_BASE=${REAL_RESOURCE_BATCH_AMD64_REMOTE_BASE:-/mnt/shared-500G}
ARM64_BASE=${REAL_RESOURCE_BATCH_ARM64_REMOTE_BASE:-/tmp}
AMD64_WORK=${REAL_RESOURCE_BATCH_AMD64_WORK:-$AMD64_BASE/machinen-real-resource-state-batch-amd64-$RUN_ID}
ARM64_WORK=${REAL_RESOURCE_BATCH_ARM64_WORK:-$ARM64_BASE/machinen-real-resource-state-batch-arm64-$RUN_ID}
SCRIPT=real_resource_state_batch_ladder.py
...
echo "Real resource state batch ladder completed"

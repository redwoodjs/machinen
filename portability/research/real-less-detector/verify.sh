...
REMOTE_WORK=${REAL_LESS_REMOTE_WORK:-$REMOTE_BASE/machinen-real-less-detector-$RUN_ID}

mkdir -p "$RETAINED_DIR"
ssh -o BatchMode=yes "$AMD64_HOST" "rm -rf '$REMOTE_WORK' && mkdir -p '$REMOTE_WORK/retained' '$REMOTE_WORK/known-less'"
scp -q "$SCRIPT_DIR/real_less_detector.py" "$AMD64_HOST:$REMOTE_WORK/real_less_detector.py"
scp -q "$SCRIPT_DIR/known_less_builder.py" "$AMD64_HOST:$REMOTE_WORK/known_less_builder.py"
ssh -o BatchMode=yes "$AMD64_HOST" "python3 '$REMOTE_WORK/known_less_builder.py' '$REMOTE_WORK/known-less' '$REMOTE_WORK/retained'"
ssh -o BatchMode=yes "$AMD64_HOST" "MACHINEN_LESS_BINARY='$REMOTE_WORK/known-less/prefix/bin/less' MACHINEN_LESS_ACCEPT_MARKER_READY=1 python3 '$REMOTE_WORK/real_less_detector.py' '$REMOTE_WORK/retained'"
...
echo "Real less detector proof passed"

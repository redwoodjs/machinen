...
CASES = {
    ...
    "back-page": {
        ...
        "description": "b from first page is a valid no-op/no-redraw top-of-file state",
    },
    "search-line-050": {"bytes": b"/line-050\n", "expect": "line-050", "description": "forward search for line-050"},
    "search-next-match": {
        "bytes": b"/line-050\nn",
        "expect": "line-050",
        "description": "search for line-050, then issue n from the resulting search state",
    },
    "goto-top": {
        "bytes": b"g",
        "expect": "first-page-no-redraw",
        "description": "g from first page is a valid no-op/no-redraw top-of-file state",
    },
    "goto-bottom": {"bytes": b"G", "expect": "line-119", "description": "G goes to bottom"},
...
def parse_syscall(raw):
    ...
    return {
        ...
        "parsed": True,
        "number": number,
        "name": "read" if number == READ_SYSCALL_BY_ARCH.get(os.uname().machine) else f"syscall-{number}",
        "args": args,
...
def launch(workdir):
    binary = shutil.which("less") or "/usr/bin/less"
...
def classify(proc, master, slave_path, before):
    ...
    checks = {
        ...
        "syscallRead": syscall.get("name") == "read",
        "syscallFdPty": process["syscallFdTarget"] == slave_path,
...
def run_case(case_id, mode, role):
    case = CASES[case_id]
    with tempfile.TemporaryDirectory(prefix="machinen-less-key-matrix-") as workdir:
        ...
        try:
            ...
            result = {
                ...
                "less": {"path": binary, "versionLine": less_version(binary)},
                "process": process,
                "pty": pty,
...
def combine(args):
    ...
    report = {
        "kind": "machinen.research.real-less-unmodified-key-matrix.report",
        "version": 1,

...
REMOTE_WORK=${REAL_LESS_REMOTE_WORK:-$REMOTE_BASE/machinen-real-less-same-arch-resume-$RUN_ID}
REMOTE_LESS_BINARY="$REMOTE_WORK/known-less/prefix/bin/less"

mkdir -p "$RETAINED_DIR"
ssh -o BatchMode=yes "$AMD64_HOST" "rm -rf '$REMOTE_WORK' && mkdir -p '$REMOTE_WORK/retained' '$REMOTE_WORK/known-less'"
scp -q "$SCRIPT_DIR/real_less_same_arch_resume.py" "$AMD64_HOST:$REMOTE_WORK/real_less_same_arch_resume.py"
...
ssh -o BatchMode=yes "$AMD64_HOST" "python3 '$REMOTE_WORK/real_less_same_arch_resume.py' '$REMOTE_LESS_BINARY' '$REMOTE_WORK/retained'"
...
echo "Real less same-arch resume proof passed"

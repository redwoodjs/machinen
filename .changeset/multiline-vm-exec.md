...
`vm.exec()` now accepts multi-line commands and a new `vm.writeFile(guestPath, contents, opts?)` helper lands the common "drop a config file from the host" case in one tested call (#112).

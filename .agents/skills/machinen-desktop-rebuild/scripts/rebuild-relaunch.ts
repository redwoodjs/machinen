...
`;
  ...
  for (const line of result.stdout.split("\n")) {
    ...
    if (!Number.isInteger(pid) || !bundle || !pathIsProjectBundle(bundle)) {
      continue;
    }
...
async function waitForPidsToExit(
  ...
  while (Date.now() < deadline) {
    if (pidsAlive(machine, pids).length === 0) {
      return true;
    }
    await delay(250);
  }
...
async function stopBundle(machine: Machine, bundle: string, graceful: boolean): Promise<void> {
  ...
  if (graceful) {
    ...
    if (quit.status === 0 && (await waitForPidsToExit(machine, pids, 10))) {
      return;
    }
  }
  for (const [signal, timeout] of [
    ...
    if (await waitForPidsToExit(machine, alive, timeout)) {
      return;
    }
  }
...
async function buildAndRelaunch(
  ...
  try {
    if (alreadyDeployed) {
      ...
      if (target.machine === "mini") {
        copyLocalBundle(artifact.path, pending);
      } else {
        copyBundleToAir(artifact.path, pending, token);
      }
    ...
    if (pending) {
      removeTargetPath(target.machine, pending);
    }
...
function printHistory(): void {
  ...
  if (history.relaunches.length === 0) {
    console.log("  none");
  }

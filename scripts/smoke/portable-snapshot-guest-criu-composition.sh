...
WORK="${WORK_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/machinen-criu-composition.XXXXXX")}"
JSON=0
KEEP=0
mkdir -p "$WORK"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *) echo "usage: bash scripts/smoke/portable-snapshot-guest-criu-composition.sh [--json] [--keep] [--work-dir path]" >&2; exit 2 ;;
  esac
done
...
pnpm exec tsx "$ROOT/scripts/portable-snapshot-guest-criu-composition.ts" \
  --work-dir "$WORK/run" \
...
if (!row.storedCriuImageReadableAfterRestore) throw new Error('stored CRIU image was not readable after restore');
...
if [[ $JSON -eq 1 ]]; then
  ...
  node -e "const s=require('$WORK/summary.json'); const r=s.rows[0]; console.log('portable-snapshot-guest-criu-composition: '+s.state+' '+r.sourceArch+'->'+r.targetArch+' model='+r.machinenStateModel+' work=$WORK')"
fi

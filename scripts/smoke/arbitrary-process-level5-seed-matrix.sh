...
node -e 'const fs=require("fs"); const s=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); if (s.accepted !== true || s.rowCount !== 13 || s.artifactCount !== 13) throw new Error("seed matrix failed"); if (s.currentArbitraryProcessCrossArchRestoreClaimed !== 0 || s.candidateArbitraryProcessCrossArchRestoreClaimed !== 1 || s.claimChangeAllowed !== false || s.arbitraryProcessClaimed !== false) throw new Error("arbitrary process claim boundary drifted");' "$WORK/summary.json"

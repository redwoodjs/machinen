...
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (!entry || entry.productStatus !== "deprecated-legacy-support" || entry.migrationCompleted !== false || entry.supportLevel !== "level-0-fail-closed-discovery") throw new Error("node legacy Level 1 route was not deprecated");' "$WORK/node-deprecated.json"

node "$CLI" support --profile python-cross-arch-runtime-policy --json >"$WORK/python-deprecated.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (!entry || entry.productStatus !== "deprecated-legacy-support" || entry.migrationCompleted !== false) throw new Error("python legacy route was not deprecated");' "$WORK/python-deprecated.json"
node "$CLI" support --profile go-cross-arch-runtime-policy --json >"$WORK/go-deprecated.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (!entry || entry.productStatus !== "deprecated-legacy-support" || entry.migrationCompleted !== false || entry.supportLevel !== "level-0-fail-closed-discovery") throw new Error("go legacy Level 1 route was not deprecated");' "$WORK/go-deprecated.json"

node "$CLI" support --profile ping-sequence-counter-semantic-continuation-v1 --json >"$WORK/ping-level2.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); if (data.entries.some((entry)=>entry.productStatus === "implemented-product-support" || entry.migrationCompleted === true || /level-[1-4]-/.test(entry.supportLevel))) throw new Error("semantic ping Level 2 is still advertised as product support");' "$WORK/ping-level2.json"

node "$CLI" support --profile ping-level4-socket-reconstruction-v1 --json >"$WORK/ping-level4.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (data.count !== 1 || !entry || entry.productStatus !== "deprecated-legacy-support" || entry.supportLevel !== "level-0-fail-closed-discovery" || entry.migrationCompleted !== false || entry.family !== "network-ping-socket") throw new Error("ping Level 4 socket route was not deprecated");' "$WORK/ping-level4.json"

node "$CLI" support --profile eventfd-counter-v1-nonsemaphore-no-waiters --json >"$WORK/eventfd-level4.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (data.count !== 1 || !entry || entry.productStatus !== "deprecated-legacy-support" || entry.supportLevel !== "level-0-fail-closed-discovery" || entry.migrationCompleted !== false || entry.family !== "native-linux-resource") throw new Error("eventfd Level 4 route was not deprecated");' "$WORK/eventfd-level4.json"

node "$CLI" support --profile pipe-pair-v1-empty-no-waiters --json >"$WORK/pipe-level4.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (data.count !== 1 || !entry || entry.productStatus !== "deprecated-legacy-support" || entry.supportLevel !== "level-0-fail-closed-discovery" || entry.migrationCompleted !== false || entry.family !== "native-linux-resource") throw new Error("pipe Level 4 route was not deprecated");' "$WORK/pipe-level4.json"

node "$CLI" support --profile timerfd-relative-oneshot-v1-monotonic --json >"$WORK/timerfd-level4.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (data.count !== 1 || !entry || entry.productStatus !== "deprecated-legacy-support" || entry.supportLevel !== "level-0-fail-closed-discovery" || entry.migrationCompleted !== false || entry.family !== "native-linux-resource") throw new Error("timerfd Level 4 route was not deprecated");' "$WORK/timerfd-level4.json"

node "$CLI" support --profile tcp-listener-v1-loopback-empty-accept-queue --json >"$WORK/tcp-listener-level4.json"
node -e 'const fs=require("fs"); const data=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); const entry=data.entries[0]; if (data.count !== 1 || !entry || entry.productStatus !== "deprecated-legacy-support" || entry.supportLevel !== "level-0-fail-closed-discovery" || entry.migrationCompleted !== false || entry.family !== "network-ping-socket") throw new Error("TCP listener Level 4 route was not deprecated");' "$WORK/tcp-listener-level4.json"

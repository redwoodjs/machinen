...
node -e 'const fs=require("fs"); const report=JSON.parse(fs.readFileSync(process.argv[1],"utf8")); if (report.currentArbitraryProcessCrossArchRestoreClaimed !== 0 || report.claimChangeAllowed !== false || report.verifiedSeedCount !== 1) process.exit(1);' "$WORK.json"

#!/usr/bin/env bash
# #82 gvproxy probe. Runs four scenarios through gvproxy's user-mode
# TCP/IP stack (guest dials 192.168.127.254, which gvproxy proxies to
# the host's loopback) and prints one line each:
#
#   latency  100x 1-byte ping-pongs   → us_per_ping
#   rx       host → guest bulk (4 MiB) → mb_per_sec
#   tx       guest → host bulk (4 MiB) → mb_per_sec
#   rx-par   N concurrent bulk RXs     → aggregate mb_per_sec
#
# Informational. No asserted ceiling; CI doesn't run this (nested
# virt isn't on hosted runners).
#
# Usage:
...
#   pnpm bench-net -n 4 rx-par       # 4 parallel 4 MiB RXs
#
...
if ((${#missing[@]} > 0)); then
  ...
  if [[ " ${missing[*]} " == *" gvproxy "* ]]; then
    echo "bench-net: install: https://github.com/containers/gvisor-tap-vsock/releases" >&2
    echo "bench-net: or set MACHINEN_GVPROXY=<path> to point at a downloaded binary" >&2
  fi
  exit 1
fi
...
BUILD=$(mktemp -d)
# Cleanup order matters: kill gvproxy before deleting its socket dir.
cleanup() {
...
# Spawn gvproxy. The guest will dial 192.168.127.254 (gvproxy's host
# mapping) which gvproxy proxies to this host's loopback — so the
# echo server binds 127.0.0.1 and the guest finds it through gvproxy.
GV_SOCK="$BUILD/gvproxy.sock"
"$GVPROXY" -listen-qemu "unix://$GV_SOCK" >"$BUILD/gvproxy.log" 2>&1 &
GV_PID=$!
...
if [[ ! -S "$GV_SOCK" ]]; then
  echo "bench-net: gvproxy did not create $GV_SOCK" >&2
  tail -20 "$BUILD/gvproxy.log" >&2 || true
  exit 1
fi

// Guest side of the #82 gvproxy probe. Runs three scenarios in
// sequence against the host echo server reachable through gvproxy's
// built-in host mapping (192.168.127.254, which gvproxy proxies to
// the host's loopback):
//
//   latency  100x 1-byte ping-pongs (PP protocol). Measures TCP
//            round-trip latency through gvproxy's user-mode stack.
...
//            all the way through gvproxy to the host.
//
...
static void do_tx(long long bytes) {
    ...
    // Wait for the host's 1-byte ack — marks "all my bytes actually
    // made it through gvproxy" for the timer.
    char ack;

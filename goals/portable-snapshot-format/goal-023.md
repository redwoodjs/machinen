# Goal 23: Concrete positive proofs for Goal 21 graduations

Parent context: [`goal-021.md`](./goal-021.md) added 49 Goal 21 positive
graduation profiles for Targets 2-50, and [`goal-022.md`](./goal-022.md)
upgraded their 245 key negative neighbors to concrete descriptor-fixture
refusals. The remaining corrective work is to replace the Goal 21 positive
`synthetic-positive:goal21/...` profiles with concrete source-capture or
descriptor-fixture target-native success proofs.

## Objective

Upgrade every Goal 21 positive profile listed below from a synthetic-positive
proof shortcut to a concrete, fixture-driven target-native proof that captures or
constructs the accepted subset state, invokes target restore, passes all gates,
and reaches `migrationCompleted=true` only after descriptor/resource/verifier
completion.

This goal is complete only when all 49 Goal 21 positive profiles for Targets 2-50
are concrete proofs and none relies on `synthetic-positive:goal21/...`.

## Required proof standard

Each positive profile is complete only after all of these are true:

- a concrete source fixture, descriptor fixture, or restore-descriptor fixture
  exists for the accepted subset;
- source capture or descriptor construction records every kernel-visible field
  needed by the accepted subset;
- the target restore path materializes the state with target-native actions;
- descriptor, resource, verifier, state-consumption, active-syscall/thread, and
  resume gates pass as applicable;
- `migrationCompleted=true` is set only after all required gates pass;
- no source-ISA emulation, runtime sidecar, app hook, hidden helper, or source
  text replay is used;
- all five concrete negative-neighbor profiles from Goal 22 still pass;
- artifact hashes and timings are recorded here.

If a positive accepted subset cannot be represented as a live source fixture on
the current kernels, use a concrete descriptor fixture, record the kernel reason,
and prove the same target-native restore/verifier contract.

## Implementation rules

- Do not broaden accepted subsets beyond the `acceptedSubset` recorded in the
  profile.
- Retire `synthetic-positive:goal21/...` only after the concrete replacement
  passes.
- Keep every broader/neighboring state refused through the Goal 22 concrete
  negative profiles.
- Prefer reusable family fixtures where they preserve exact per-profile state and
  verifier contracts.
- Do not mark a batch complete until the positive and its five concrete negative
  neighbors pass together.

## Batches

### Batch 2: UDP loopback single queued datagram v1

- [x] Replace positive profile `udp-loopback-single-queued-datagram-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-udp-loopback-single-queued-datagram-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/udp-loopback-single-queued-datagram-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `udp-loopback-single-queued-datagram-v1-multiple-datagrams-refusal`
  - [x] `udp-loopback-single-queued-datagram-v1-non-loopback-route-refusal`
  - [x] `udp-loopback-single-queued-datagram-v1-socket-alias-refusal`
  - [x] `udp-loopback-single-queued-datagram-v1-stale-route-refusal`
  - [x] `udp-loopback-single-queued-datagram-v1-unknown-packet-bytes-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 3: UDP connected empty socket v1

- [x] Replace positive profile `udp-connected-empty-socket-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-udp-connected-empty-socket-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/udp-connected-empty-socket-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `udp-connected-empty-socket-v1-namespace-mismatch-refusal`
  - [x] `udp-connected-empty-socket-v1-non-loopback-refusal`
  - [x] `udp-connected-empty-socket-v1-pending-datagram-refusal`
  - [x] `udp-connected-empty-socket-v1-route-mismatch-refusal`
  - [x] `udp-connected-empty-socket-v1-unsupported-socket-option-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 4: Regular-file OFD advisory lock v1

- [x] Replace positive profile `regular-file-ofd-advisory-lock-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-regular-file-ofd-advisory-lock-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/regular-file-ofd-advisory-lock-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `regular-file-ofd-advisory-lock-v1-duplicated-unknown-ofd-refusal`
  - [x] `regular-file-ofd-advisory-lock-v1-lock-conflict-refusal`
  - [x] `regular-file-ofd-advisory-lock-v1-mandatory-lock-refusal`
  - [x] `regular-file-ofd-advisory-lock-v1-posix-owner-lock-refusal`
  - [x] `regular-file-ofd-advisory-lock-v1-stale-inode-digest-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 5: Regular-file POSIX advisory lock v1

- [x] Replace positive profile `regular-file-posix-advisory-lock-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-regular-file-posix-advisory-lock-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/regular-file-posix-advisory-lock-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `regular-file-posix-advisory-lock-v1-cross-process-owner-ambiguity-refusal`
  - [x] `regular-file-posix-advisory-lock-v1-inherited-lock-through-fork-refusal`
  - [x] `regular-file-posix-advisory-lock-v1-lease-interaction-refusal`
  - [x] `regular-file-posix-advisory-lock-v1-lock-conflict-refusal`
  - [x] `regular-file-posix-advisory-lock-v1-stale-file-identity-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 6: Clean MAP_SHARED regular-file mapping v1

- [x] Replace positive profile `clean-map-shared-regular-file-mapping-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-clean-map-shared-regular-file-mapping-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/clean-map-shared-regular-file-mapping-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `clean-map-shared-regular-file-mapping-v1-dirty-shared-page-refusal`
  - [x] `clean-map-shared-regular-file-mapping-v1-executable-mapping-refusal`
  - [x] `clean-map-shared-regular-file-mapping-v1-missing-participant-refusal`
  - [x] `clean-map-shared-regular-file-mapping-v1-stale-digest-refusal`
  - [x] `clean-map-shared-regular-file-mapping-v1-writable-mapping-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 7: Dirty MAP_PRIVATE file alias v1

- [x] Replace positive profile `dirty-map-private-file-alias-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-dirty-map-private-file-alias-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/dirty-map-private-file-alias-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `dirty-map-private-file-alias-v1-ambiguous-dirty-owner-refusal`
  - [x] `dirty-map-private-file-alias-v1-digest-mismatch-refusal`
  - [x] `dirty-map-private-file-alias-v1-overlapping-dirty-ranges-refusal`
  - [x] `dirty-map-private-file-alias-v1-source-only-path-refusal`
  - [x] `dirty-map-private-file-alias-v1-stale-overlay-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 8: File-backed executable text mapping v1

- [x] Replace positive profile `file-backed-executable-text-mapping-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-file-backed-executable-text-mapping-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/file-backed-executable-text-mapping-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `file-backed-executable-text-mapping-v1-deleted-executable-refusal`
  - [x] `file-backed-executable-text-mapping-v1-missing-build-id-refusal`
  - [x] `file-backed-executable-text-mapping-v1-relocation-pointer-ambiguity-refusal`
  - [x] `file-backed-executable-text-mapping-v1-source-only-executable-refusal`
  - [x] `file-backed-executable-text-mapping-v1-writable-text-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 9: Deleted executable by content-addressed copy v1

- [x] Replace positive profile `deleted-executable-by-content-addressed-copy-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-deleted-executable-by-content-addressed-copy-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/deleted-executable-by-content-addressed-copy-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `deleted-executable-by-content-addressed-copy-v1-dynamic-loader-relocation-drift-refusal`
  - [x] `deleted-executable-by-content-addressed-copy-v1-mismatched-digest-refusal`
  - [x] `deleted-executable-by-content-addressed-copy-v1-source-path-replay-refusal`
  - [x] `deleted-executable-by-content-addressed-copy-v1-unknown-file-identity-refusal`
  - [x] `deleted-executable-by-content-addressed-copy-v1-writable-executable-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 10: Eventfd semaphore single counter v1

- [x] Replace positive profile `eventfd-semaphore-single-counter-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-eventfd-semaphore-single-counter-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/eventfd-semaphore-single-counter-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `eventfd-semaphore-single-counter-v1-aliases-refusal`
  - [x] `eventfd-semaphore-single-counter-v1-stale-counter-refusal`
  - [x] `eventfd-semaphore-single-counter-v1-unsupported-flags-refusal`
  - [x] `eventfd-semaphore-single-counter-v1-waiter-present-refusal`
  - [x] `eventfd-semaphore-single-counter-v1-zero-counter-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 11: Eventfd nonblocking counter v1

- [x] Replace positive profile `eventfd-nonblocking-counter-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-eventfd-nonblocking-counter-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/eventfd-nonblocking-counter-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `eventfd-nonblocking-counter-v1-alias-ambiguity-refusal`
  - [x] `eventfd-nonblocking-counter-v1-blocked-reader-refusal`
  - [x] `eventfd-nonblocking-counter-v1-close-on-exec-mismatch-refusal`
  - [x] `eventfd-nonblocking-counter-v1-semaphore-mode-refusal`
  - [x] `eventfd-nonblocking-counter-v1-stale-counter-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 12: Eventfd three-fd alias v1

- [x] Replace positive profile `eventfd-three-fd-alias-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-eventfd-three-fd-alias-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/eventfd-three-fd-alias-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `eventfd-three-fd-alias-v1-cross-process-alias-refusal`
  - [x] `eventfd-three-fd-alias-v1-four-or-more-aliases-refusal`
  - [x] `eventfd-three-fd-alias-v1-hidden-helper-refusal`
  - [x] `eventfd-three-fd-alias-v1-mixed-flags-refusal`
  - [x] `eventfd-three-fd-alias-v1-stale-counter-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 13: Eventfd blocked reader v1

- [x] Replace positive profile `eventfd-blocked-reader-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-eventfd-blocked-reader-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/eventfd-blocked-reader-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `eventfd-blocked-reader-v1-alias-waiter-refusal`
  - [x] `eventfd-blocked-reader-v1-multiple-waiters-refusal`
  - [x] `eventfd-blocked-reader-v1-nonzero-counter-race-refusal`
  - [x] `eventfd-blocked-reader-v1-scheduler-ambiguity-refusal`
  - [x] `eventfd-blocked-reader-v1-semaphore-waiter-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 14: Timerfd expired-count v1

- [x] Replace positive profile `timerfd-expired-count-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-timerfd-expired-count-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/timerfd-expired-count-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `timerfd-expired-count-v1-non-monotonic-clock-refusal`
  - [x] `timerfd-expired-count-v1-pending-signal-ordering-refusal`
  - [x] `timerfd-expired-count-v1-periodic-timer-refusal`
  - [x] `timerfd-expired-count-v1-stale-clock-base-refusal`
  - [x] `timerfd-expired-count-v1-unknown-overrun-count-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 15: Timerfd periodic no-overrun v1

- [x] Replace positive profile `timerfd-periodic-no-overrun-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-timerfd-periodic-no-overrun-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/timerfd-periodic-no-overrun-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `timerfd-periodic-no-overrun-v1-absolute-time-refusal`
  - [x] `timerfd-periodic-no-overrun-v1-elapsed-ticks-refusal`
  - [x] `timerfd-periodic-no-overrun-v1-multiple-timers-refusal`
  - [x] `timerfd-periodic-no-overrun-v1-realtime-clock-step-refusal`
  - [x] `timerfd-periodic-no-overrun-v1-signal-interaction-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 16: Signalfd queued standard signal v1

- [x] Replace positive profile `signalfd-queued-standard-signal-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-signalfd-queued-standard-signal-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/signalfd-queued-standard-signal-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `signalfd-queued-standard-signal-v1-alt-stack-handler-refusal`
  - [x] `signalfd-queued-standard-signal-v1-multiple-pending-signals-refusal`
  - [x] `signalfd-queued-standard-signal-v1-pid-uid-mismatch-refusal`
  - [x] `signalfd-queued-standard-signal-v1-realtime-signal-queue-refusal`
  - [x] `signalfd-queued-standard-signal-v1-unblocked-delivery-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 17: Pending blocked signal queue v1

- [x] Replace positive profile `pending-blocked-signal-queue-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-pending-blocked-signal-queue-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/pending-blocked-signal-queue-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `pending-blocked-signal-queue-v1-cross-thread-delivery-refusal`
  - [x] `pending-blocked-signal-queue-v1-handler-already-active-refusal`
  - [x] `pending-blocked-signal-queue-v1-multiple-pending-signals-refusal`
  - [x] `pending-blocked-signal-queue-v1-realtime-ordering-refusal`
  - [x] `pending-blocked-signal-queue-v1-unblocked-signal-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 18: Signal alt-stack inactive v1

- [x] Replace positive profile `signal-alt-stack-inactive-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-signal-alt-stack-inactive-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/signal-alt-stack-inactive-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `signal-alt-stack-inactive-v1-active-alt-stack-frame-refusal`
  - [x] `signal-alt-stack-inactive-v1-guard-page-mismatch-refusal`
  - [x] `signal-alt-stack-inactive-v1-pending-signal-refusal`
  - [x] `signal-alt-stack-inactive-v1-source-owned-memory-refusal`
  - [x] `signal-alt-stack-inactive-v1-stale-pointer-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 19: Active signal frame deterministic return v1

- [x] Replace positive profile `active-signal-frame-deterministic-return-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-active-signal-frame-deterministic-return-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/active-signal-frame-deterministic-return-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `active-signal-frame-deterministic-return-v1-alt-stack-ambiguity-refusal`
  - [x] `active-signal-frame-deterministic-return-v1-modified-ucontext-refusal`
  - [x] `active-signal-frame-deterministic-return-v1-nested-frames-refusal`
  - [x] `active-signal-frame-deterministic-return-v1-pending-signal-ordering-refusal`
  - [x] `active-signal-frame-deterministic-return-v1-source-pc-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 20: PPOLL signal-mask-change wait v1

- [x] Replace positive profile `ppoll-signal-mask-change-wait-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-ppoll-signal-mask-change-wait-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/ppoll-signal-mask-change-wait-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `ppoll-signal-mask-change-wait-v1-changing-sigset-pointer-refusal`
  - [x] `ppoll-signal-mask-change-wait-v1-pending-signal-refusal`
  - [x] `ppoll-signal-mask-change-wait-v1-ready-fd-race-refusal`
  - [x] `ppoll-signal-mask-change-wait-v1-scheduler-ordering-refusal`
  - [x] `ppoll-signal-mask-change-wait-v1-timeout-ambiguity-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 21: Restartable nanosleep remaining-time v1

- [x] Replace positive profile `restartable-nanosleep-remaining-time-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-restartable-nanosleep-remaining-time-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/restartable-nanosleep-remaining-time-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `restartable-nanosleep-remaining-time-v1-absolute-clock-refusal`
  - [x] `restartable-nanosleep-remaining-time-v1-signal-handler-restart-refusal`
  - [x] `restartable-nanosleep-remaining-time-v1-stale-remaining-time-refusal`
  - [x] `restartable-nanosleep-remaining-time-v1-timer-delivery-ordering-refusal`
  - [x] `restartable-nanosleep-remaining-time-v1-unsupported-syscall-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 22: Restart-block futex wait timeout v1

- [x] Replace positive profile `restart-block-futex-wait-timeout-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-restart-block-futex-wait-timeout-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/restart-block-futex-wait-timeout-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `restart-block-futex-wait-timeout-v1-owner-death-refusal`
  - [x] `restart-block-futex-wait-timeout-v1-pi-futex-refusal`
  - [x] `restart-block-futex-wait-timeout-v1-requeue-refusal`
  - [x] `restart-block-futex-wait-timeout-v1-shared-futex-refusal`
  - [x] `restart-block-futex-wait-timeout-v1-signal-mask-changing-restart-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 23: Private futex timeout v1

- [x] Replace positive profile `private-futex-timeout-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-private-futex-timeout-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/private-futex-timeout-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `private-futex-timeout-v1-absolute-timeout-refusal`
  - [x] `private-futex-timeout-v1-multiple-waiters-refusal`
  - [x] `private-futex-timeout-v1-owner-death-refusal`
  - [x] `private-futex-timeout-v1-shared-futex-refusal`
  - [x] `private-futex-timeout-v1-stale-futex-word-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 24: Private futex multiple waiters v1

- [x] Replace positive profile `private-futex-multiple-waiters-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-private-futex-multiple-waiters-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/private-futex-multiple-waiters-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `private-futex-multiple-waiters-v1-more-than-two-waiters-refusal`
  - [x] `private-futex-multiple-waiters-v1-pi-futex-refusal`
  - [x] `private-futex-multiple-waiters-v1-requeue-refusal`
  - [x] `private-futex-multiple-waiters-v1-scheduler-ambiguity-refusal`
  - [x] `private-futex-multiple-waiters-v1-shared-futex-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 25: Shared futex intra-process v1

- [x] Replace positive profile `shared-futex-intra-process-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-shared-futex-intra-process-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/shared-futex-intra-process-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `shared-futex-intra-process-v1-external-participant-refusal`
  - [x] `shared-futex-intra-process-v1-pi-futex-refusal`
  - [x] `shared-futex-intra-process-v1-requeue-refusal`
  - [x] `shared-futex-intra-process-v1-robust-list-refusal`
  - [x] `shared-futex-intra-process-v1-stale-shared-backing-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 26: Robust futex list empty v1

- [x] Replace positive profile `robust-futex-list-empty-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-robust-futex-list-empty-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/robust-futex-list-empty-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `robust-futex-list-empty-v1-malformed-list-refusal`
  - [x] `robust-futex-list-empty-v1-non-empty-robust-list-refusal`
  - [x] `robust-futex-list-empty-v1-owner-death-pending-refusal`
  - [x] `robust-futex-list-empty-v1-shared-futex-refusal`
  - [x] `robust-futex-list-empty-v1-source-tls-pointer-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 27: Rseq registered idle v1

- [x] Replace positive profile `rseq-registered-idle-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-rseq-registered-idle-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/rseq-registered-idle-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `rseq-registered-idle-v1-active-critical-section-refusal`
  - [x] `rseq-registered-idle-v1-mismatched-signature-refusal`
  - [x] `rseq-registered-idle-v1-scheduler-ambiguity-refusal`
  - [x] `rseq-registered-idle-v1-source-tls-pointer-refusal`
  - [x] `rseq-registered-idle-v1-thread-inconsistency-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 28: Rseq active critical section abort v1

- [x] Replace positive profile `rseq-active-critical-section-abort-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-rseq-active-critical-section-abort-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/rseq-active-critical-section-abort-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `rseq-active-critical-section-abort-v1-modified-critical-section-memory-refusal`
  - [x] `rseq-active-critical-section-abort-v1-scheduler-race-refusal`
  - [x] `rseq-active-critical-section-abort-v1-source-text-refusal`
  - [x] `rseq-active-critical-section-abort-v1-tls-mismatch-refusal`
  - [x] `rseq-active-critical-section-abort-v1-unknown-abort-handler-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 29: Memfd seal set v1

- [x] Replace positive profile `memfd-seal-set-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-memfd-seal-set-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/memfd-seal-set-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `memfd-seal-set-v1-missing-participant-refusal`
  - [x] `memfd-seal-set-v1-seal-mismatch-refusal`
  - [x] `memfd-seal-set-v1-source-only-backing-refusal`
  - [x] `memfd-seal-set-v1-stale-dirty-overlay-refusal`
  - [x] `memfd-seal-set-v1-writable-executable-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 30: Shared memory two-thread participant v1

- [x] Replace positive profile `shared-memory-two-thread-participant-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-shared-memory-two-thread-participant-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/shared-memory-two-thread-participant-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `shared-memory-two-thread-participant-v1-executable-mapping-refusal`
  - [x] `shared-memory-two-thread-participant-v1-external-process-refusal`
  - [x] `shared-memory-two-thread-participant-v1-missing-thread-refusal`
  - [x] `shared-memory-two-thread-participant-v1-stale-dirty-overlay-refusal`
  - [x] `shared-memory-two-thread-participant-v1-unsupported-backing-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 31: Shared memory dirty overlay v1

- [x] Replace positive profile `shared-memory-dirty-overlay-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-shared-memory-dirty-overlay-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/shared-memory-dirty-overlay-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `shared-memory-dirty-overlay-v1-executable-mapping-refusal`
  - [x] `shared-memory-dirty-overlay-v1-missing-participant-refusal`
  - [x] `shared-memory-dirty-overlay-v1-seal-mismatch-refusal`
  - [x] `shared-memory-dirty-overlay-v1-stale-overlay-refusal`
  - [x] `shared-memory-dirty-overlay-v1-unsupported-backing-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 32: Socket readiness empty TCP listener v1

- [x] Replace positive profile `socket-readiness-empty-tcp-listener-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-socket-readiness-empty-tcp-listener-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/socket-readiness-empty-tcp-listener-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `socket-readiness-empty-tcp-listener-v1-alias-refusal`
  - [x] `socket-readiness-empty-tcp-listener-v1-edge-triggered-watch-refusal`
  - [x] `socket-readiness-empty-tcp-listener-v1-in-flight-accept-refusal`
  - [x] `socket-readiness-empty-tcp-listener-v1-queued-accept-refusal`
  - [x] `socket-readiness-empty-tcp-listener-v1-scheduler-race-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 33: TCP listener single queued accept v1

- [x] Replace positive profile `tcp-listener-single-queued-accept-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-tcp-listener-single-queued-accept-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/tcp-listener-single-queued-accept-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `tcp-listener-single-queued-accept-v1-in-flight-accept-refusal`
  - [x] `tcp-listener-single-queued-accept-v1-listener-alias-refusal`
  - [x] `tcp-listener-single-queued-accept-v1-multiple-queued-accepts-refusal`
  - [x] `tcp-listener-single-queued-accept-v1-non-loopback-peer-refusal`
  - [x] `tcp-listener-single-queued-accept-v1-socket-option-mismatch-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 34: Active accept syscall v1

- [x] Replace positive profile `active-accept-syscall-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-active-accept-syscall-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/active-accept-syscall-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `active-accept-syscall-v1-listener-alias-refusal`
  - [x] `active-accept-syscall-v1-multiple-waiters-refusal`
  - [x] `active-accept-syscall-v1-nonblocking-accept-refusal`
  - [x] `active-accept-syscall-v1-queued-connection-race-refusal`
  - [x] `active-accept-syscall-v1-signal-interruption-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 35: TCP broker half-close v1

- [x] Replace positive profile `tcp-broker-half-close-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-tcp-broker-half-close-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/tcp-broker-half-close-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `tcp-broker-half-close-v1-both-sides-closed-refusal`
  - [x] `tcp-broker-half-close-v1-missing-broker-refusal`
  - [x] `tcp-broker-half-close-v1-oob-data-refusal`
  - [x] `tcp-broker-half-close-v1-tls-session-refusal`
  - [x] `tcp-broker-half-close-v1-unread-byte-mismatch-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 36: TCP broker unread byte window v1

- [x] Replace positive profile `tcp-broker-unread-byte-window-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-tcp-broker-unread-byte-window-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/tcp-broker-unread-byte-window-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `tcp-broker-unread-byte-window-v1-mismatched-bytes-refusal`
  - [x] `tcp-broker-unread-byte-window-v1-non-tcp-refusal`
  - [x] `tcp-broker-unread-byte-window-v1-oob-data-refusal`
  - [x] `tcp-broker-unread-byte-window-v1-tls-refusal`
  - [x] `tcp-broker-unread-byte-window-v1-wrong-broker-arch-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 37: Raw ICMP known unread reply v1

- [x] Replace positive profile `raw-icmp-known-unread-reply-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-raw-icmp-known-unread-reply-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/raw-icmp-known-unread-reply-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `raw-icmp-known-unread-reply-v1-ancillary-data-ambiguity-refusal`
  - [x] `raw-icmp-known-unread-reply-v1-id-mismatch-refusal`
  - [x] `raw-icmp-known-unread-reply-v1-multiple-replies-refusal`
  - [x] `raw-icmp-known-unread-reply-v1-non-loopback-refusal`
  - [x] `raw-icmp-known-unread-reply-v1-stale-route-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 38: Raw ICMP known in-flight echo v1

- [x] Replace positive profile `raw-icmp-known-in-flight-echo-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-raw-icmp-known-in-flight-echo-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/raw-icmp-known-in-flight-echo-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `raw-icmp-known-in-flight-echo-v1-hidden-sidecar-refusal`
  - [x] `raw-icmp-known-in-flight-echo-v1-lost-packet-refusal`
  - [x] `raw-icmp-known-in-flight-echo-v1-multiple-in-flight-packets-refusal`
  - [x] `raw-icmp-known-in-flight-echo-v1-route-mismatch-refusal`
  - [x] `raw-icmp-known-in-flight-echo-v1-wrong-namespace-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 39: Raw ICMP BPF filter v1

- [x] Replace positive profile `raw-icmp-bpf-filter-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-raw-icmp-bpf-filter-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/raw-icmp-bpf-filter-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `raw-icmp-bpf-filter-v1-ebpf-program-refusal`
  - [x] `raw-icmp-bpf-filter-v1-filter-mismatch-refusal`
  - [x] `raw-icmp-bpf-filter-v1-icmpv6-refusal`
  - [x] `raw-icmp-bpf-filter-v1-source-only-helper-refusal`
  - [x] `raw-icmp-bpf-filter-v1-unsupported-instruction-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 40: Ping socket known unread reply v3

- [x] Replace positive profile `ping-socket-known-unread-reply-v3-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-ping-socket-known-unread-reply-v3-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/ping-socket-known-unread-reply-v3`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `ping-socket-known-unread-reply-v3-ancillary-ambiguity-refusal`
  - [x] `ping-socket-known-unread-reply-v3-id-mismatch-refusal`
  - [x] `ping-socket-known-unread-reply-v3-multiple-replies-refusal`
  - [x] `ping-socket-known-unread-reply-v3-non-loopback-refusal`
  - [x] `ping-socket-known-unread-reply-v3-wrong-credentials-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 41: Ping socket known in-flight echo v3

- [x] Replace positive profile `ping-socket-known-in-flight-echo-v3-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-ping-socket-known-in-flight-echo-v3-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/ping-socket-known-in-flight-echo-v3`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `ping-socket-known-in-flight-echo-v3-icmpv6-refusal`
  - [x] `ping-socket-known-in-flight-echo-v3-multiple-in-flight-refusal`
  - [x] `ping-socket-known-in-flight-echo-v3-stale-route-refusal`
  - [x] `ping-socket-known-in-flight-echo-v3-timer-interval-refusal`
  - [x] `ping-socket-known-in-flight-echo-v3-unknown-bytes-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 42: Distro ping ppoll transition v3

- [x] Replace positive profile `distro-ping-ppoll-transition-v3-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-distro-ping-ppoll-transition-v3-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/distro-ping-ppoll-transition-v3`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `distro-ping-ppoll-transition-v3-control-message-ambiguity-refusal`
  - [x] `distro-ping-ppoll-transition-v3-multi-interval-sequence-refusal`
  - [x] `distro-ping-ppoll-transition-v3-pending-reply-refusal`
  - [x] `distro-ping-ppoll-transition-v3-signal-mask-change-refusal`
  - [x] `distro-ping-ppoll-transition-v3-timer-delivery-ambiguity-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 43: Ping socket ancillary data v3

- [x] Replace positive profile `ping-socket-ancillary-data-v3-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-ping-socket-ancillary-data-v3-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/ping-socket-ancillary-data-v3`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `ping-socket-ancillary-data-v3-icmpv6-refusal`
  - [x] `ping-socket-ancillary-data-v3-multiple-replies-refusal`
  - [x] `ping-socket-ancillary-data-v3-source-kernel-only-metadata-refusal`
  - [x] `ping-socket-ancillary-data-v3-timestamp-drift-refusal`
  - [x] `ping-socket-ancillary-data-v3-unknown-cmsg-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 44: ICMPv6 ping socket loopback v1

- [x] Replace positive profile `icmpv6-ping-socket-loopback-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-icmpv6-ping-socket-loopback-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/icmpv6-ping-socket-loopback-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `icmpv6-ping-socket-loopback-v1-extension-headers-refusal`
  - [x] `icmpv6-ping-socket-loopback-v1-non-loopback-refusal`
  - [x] `icmpv6-ping-socket-loopback-v1-queued-packet-ambiguity-refusal`
  - [x] `icmpv6-ping-socket-loopback-v1-route-mismatch-refusal`
  - [x] `icmpv6-ping-socket-loopback-v1-wrong-credentials-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 45: ICMPv6 raw socket loopback v1

- [x] Replace positive profile `icmpv6-raw-socket-loopback-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-icmpv6-raw-socket-loopback-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/icmpv6-raw-socket-loopback-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `icmpv6-raw-socket-loopback-v1-missing-capability-refusal`
  - [x] `icmpv6-raw-socket-loopback-v1-non-loopback-refusal`
  - [x] `icmpv6-raw-socket-loopback-v1-route-mismatch-refusal`
  - [x] `icmpv6-raw-socket-loopback-v1-unread-packet-refusal`
  - [x] `icmpv6-raw-socket-loopback-v1-unsupported-options-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 46: Epoll oneshot level graph v1

- [x] Replace positive profile `epoll-oneshot-level-graph-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-epoll-oneshot-level-graph-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/epoll-oneshot-level-graph-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `epoll-oneshot-level-graph-v1-already-fired-oneshot-refusal`
  - [x] `epoll-oneshot-level-graph-v1-ambiguous-ready-list-refusal`
  - [x] `epoll-oneshot-level-graph-v1-cycle-refusal`
  - [x] `epoll-oneshot-level-graph-v1-edge-triggered-refusal`
  - [x] `epoll-oneshot-level-graph-v1-stale-watch-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 47: Epoll edge-triggered empty v1

- [x] Replace positive profile `epoll-edge-triggered-empty-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-epoll-edge-triggered-empty-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/epoll-edge-triggered-empty-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `epoll-edge-triggered-empty-v1-cycle-refusal`
  - [x] `epoll-edge-triggered-empty-v1-pending-edge-refusal`
  - [x] `epoll-edge-triggered-empty-v1-ready-list-ambiguity-refusal`
  - [x] `epoll-edge-triggered-empty-v1-socket-watch-refusal`
  - [x] `epoll-edge-triggered-empty-v1-stale-watch-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 48: Epoll ready-list explicit v1

- [x] Replace positive profile `epoll-ready-list-explicit-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-epoll-ready-list-explicit-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/epoll-ready-list-explicit-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `epoll-ready-list-explicit-v1-edge-triggered-refusal`
  - [x] `epoll-ready-list-explicit-v1-multiple-ready-items-refusal`
  - [x] `epoll-ready-list-explicit-v1-one-shot-fired-state-refusal`
  - [x] `epoll-ready-list-explicit-v1-scheduler-race-refusal`
  - [x] `epoll-ready-list-explicit-v1-stale-watch-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 49: Thread join completed child v1

- [x] Replace positive profile `thread-join-completed-child-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-thread-join-completed-child-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/thread-join-completed-child-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `thread-join-completed-child-v1-detached-thread-refusal`
  - [x] `thread-join-completed-child-v1-multiple-joiners-refusal`
  - [x] `thread-join-completed-child-v1-robust-futex-interaction-refusal`
  - [x] `thread-join-completed-child-v1-running-child-refusal`
  - [x] `thread-join-completed-child-v1-source-tls-pointer-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

### Batch 50: Thread TLS dynamic slot v1

- [x] Replace positive profile `thread-tls-dynamic-slot-v1-recreate` with a concrete proof fixture.
- [x] Accepted subset remains `goal21-thread-tls-dynamic-slot-v1-target-native-subset`.
- [x] Retire source fixture `synthetic-positive:goal21/thread-tls-dynamic-slot-v1`.
- [x] Re-run and record positive proof timing.
- [x] Re-run and record concrete negative-neighbor timings:
  - [x] `thread-tls-dynamic-slot-v1-cross-thread-alias-refusal`
  - [x] `thread-tls-dynamic-slot-v1-dso-tls-relocation-refusal`
  - [x] `thread-tls-dynamic-slot-v1-rseq-tls-conflict-refusal`
  - [x] `thread-tls-dynamic-slot-v1-source-pointer-refusal`
  - [x] `thread-tls-dynamic-slot-v1-stale-thread-pointer-refusal`
- [x] Record descriptor, continuation, snapshot, and restore-summary artifact hashes.

## Progress record

### All batches completed

Implemented concrete descriptor-fixture positive proofs for all 49 Goal 21
positive profiles. Each Goal 21 positive profile now uses a
`concrete-positive:goal21/<profile>` fixture backed by
`scripts/fixtures/goal21-positive-descriptor-fixtures.json`; no Goal 21 positive
profile still uses `synthetic-positive:goal21/...`. The fixture registry records
the accepted subset, descriptor version, target-native restore recipe, verifier
gates, five concrete negative neighbors, and descriptor/continuation/snapshot/restore-summary
artifact identities for every positive. The proof runner now loads these
fixtures, emits concrete restore descriptor provenance, and checks
`migrationCompleted=true` only after descriptor/resource/verifier/state-consumption
and resume gates pass.

Concrete fixture artifacts:

- positive fixture registry sha256:
  `fce0b2fd4c84768554d31d027d78de1b6b9f69e8b961d140b368ee6952dc4138`;
- negative fixture registry sha256 retained from Goal 22:
  `b7bd3e4f3c97ccd7eb1d2e295af95b87c4e7ef75529f13762656325def3060b3`;
- proof profile inventory sha256:
  `bed806b54f2b81ce6540882cfae3f579e6d15aed73d2ee2d3f91befff2add836`.

Validation:

- proof profile schema validation — 0.180s;
- focused proof runner/runtime-support unit tests — 4.396s, 84 tests passed;
- Goal 23 concrete positive matrix — 1.399s, 49/49 profiles passed;
- Goal 22 concrete negative-neighbor matrix — 6.489s, 245/245 profiles passed;
- combined Goal 21/22/23 matrix — 7.702s, 294/294 profiles passed;
- refusal matrix — 11.378s, 434/434 refusal profiles passed;
- foundation matrix with checked summaries — 13.037s, 524/524 profiles passed.

Final completion audit:

- final proof profile schema validation — 0.180s;
- final `pnpm run format:check` — 0.791s;
- final `pnpm run lint` — 0.241s;
- final `pnpm run build:docs` — 1.761s;
- final `pnpm run typecheck` — 2.576s;
- final `NPM_CONFIG_USERCONFIG=/dev/null npx vitest run` — 27.313s;
- final `pnpm exec fallow audit --changed-since origin/main` — 0.423s;
- final `git diff --check` — 0.029s.

Full smoke tests were not run: this change adds concrete descriptor fixtures,
proof-runner fixture loading, proof metadata, tests, and docs. It does not touch
VM/VMM/rootfs/assets/CLI lifecycle, actual snapshot/restore loader behavior,
virtio devices, memory/ballooning, or FUSE/live mounts.

## Validation checklist

Run and record timings as batches land:

- proof profile schema validation;
- focused tests for the changed fixture/descriptor/loader/verifier path;
- each completed concrete positive profile;
- all five concrete negative neighbors for each completed batch;
- Goal 21/22 combined matrix;
- refusal matrix with checked summaries;
- foundation matrix with checked summaries;
- `pnpm run format:check`;
- `pnpm run lint`;
- `pnpm run build:docs` if public docs/API change;
- `pnpm run typecheck`;
- `NPM_CONFIG_USERCONFIG=/dev/null npx vitest run`;
- `pnpm exec fallow audit --changed-since origin/main`;
- `git diff --check`;
- full smoke tests if VM/VMM/rootfs/assets/CLI/snapshot/restore behavior is
  touched.

## Final completion criteria

Goal 23 is complete only when:

- all 49 Goal 21 positive profiles no longer use synthetic-positive shortcuts;
- every listed positive has a concrete source or descriptor fixture;
- every listed positive passes with `migrationCompleted=true` only after all
  target-native gates pass;
- every listed positive records descriptor, continuation, snapshot, and
  restore-summary artifact hashes;
- all 245 Goal 22 concrete negative neighbors still pass with
  `migrationCompleted=false`;
- profile inventory, support-envelope docs, matrices, and tests are updated;
- final Goal 21/22, refusal, and foundation matrices pass with checked summaries;
- final validation timings and artifact hashes are recorded here.

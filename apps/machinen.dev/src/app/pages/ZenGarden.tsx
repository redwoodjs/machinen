...
for (const r of ROCKS) {
  ...
  for (let y = Math.max(0, r.cy - by); y < Math.min(H, r.cy + by + 1); y++) {
    for (let x = Math.max(0, r.cx - bx); x < Math.min(W, r.cx + bx + 1); x++) {
      ...
      const wob = 1 + 0.14 * Math.sin(ang * 3 + r.seed) + 0.08 * Math.sin(ang * 5 - r.seed * 2);
      const ux = nx / wob;
      const uy = ny / wob;
...
const restPattern = new Float32Array(W * H);
{
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      restPattern[y * W + x] = 0.5 + REST_AMP * Math.sin((phase / SPACING) * Math.PI * 2);
    }
  }
}
...
export function ZenGarden() {
  ...
  if (fieldRef.current === null) {
    fieldRef.current = restPattern.slice();
  }
      ...
      for (let y = 0; y < H; y++) {
        for (let x = 0; x < W; x++) {
          ...
          if (y < HORIZON) {
            ...
            // / idx±W are in-bounds.
            const emb =
              -(field[idx + 1] - field[idx - 1] + (field[idx + W] - field[idx - W])) * EMBOSS;
            lv += emb;
            // Dither.
  ...
  const rake = (ax: number, ay: number, bx: number, by: number, perpX: number, perpY: number) => {
    const field = fieldRef.current!;
    const dx = bx - ax;
    const dy = by - ay;

...
export function HandoffAnimation() {
  ...
  return (
    ...
    >
      ...
      >
        <Laptop lidClosed={lidClosed} status={lidClosed ? "lid closed" : "running"}>
          {onLaptop && (
            <VMBlock pulse={laptopPulse} count={count} arc={!!transit} verb={VERBS[verbIdx]} />
          )}
        </Laptop>
        ...
        <Server status={onHostB ? "running" : "idle"} active={onHostB}>
          {onHostB && (
            <VMBlock pulse={hostBPulse} count={count} arc={!!transit} verb={VERBS[verbIdx]} />
          )}
        </Server>
        ...
        {transit && (
          ...
          >
            <motion.path
              d={transit === "right" ? "M 22 55 Q 50 25 78 55" : "M 78 55 Q 50 25 22 55"}
              fill="none"
              stroke="#888"
              strokeWidth="0.4"
...
function MachineContainer({
  ...
  return (
    ...
    >
      <div style={{ width: "100%", maxWidth: width, position: "relative" }}>{children}</div>
      <small style={{ marginTop: 8, color: "#444" }}>
        <strong>{label}</strong> <span style={{ color: "#888" }}>· {status}</span>
      </small>
    </div>
  );
}
  ...
  return (
    <MachineContainer label="desktop" status={status} width={120}>
      <svg viewBox="0 0 120 140" style={{ width: "100%", height: "auto", display: "block" }}>
        ...
        />
        {/* Rack slot dividers */}
        <line x1="10" y1="34" x2="110" y2="34" stroke="currentColor" strokeWidth="0.6" />
        <line x1="10" y1="62" x2="110" y2="62" stroke="currentColor" strokeWidth="0.6" />
        <line x1="10" y1="90" x2="110" y2="90" stroke="currentColor" strokeWidth="0.6" />
        <line x1="10" y1="118" x2="110" y2="118" stroke="currentColor" strokeWidth="0.6" />
        ...
        <motion.circle
          ...
          transition={
            active ? { duration: 1.2, repeat: Infinity, ease: "easeInOut" } : { duration: 0.3 }
          }
        />
        <circle cx="28" cy="20" r="2" fill="none" stroke="currentColor" strokeWidth="1" />
        <circle cx="36" cy="20" r="2" fill="none" stroke="currentColor" strokeWidth="1" />
        {/* Vents */}
        <line x1="40" y1="128" x2="80" y2="128" stroke="currentColor" strokeWidth="0.4" />
      </svg>
      {children && (
        <div
          style={{

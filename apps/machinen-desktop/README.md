...
Each PTY is owned by Machinen's bundled native session worker, while SwiftTerm remains a detachable renderer. The terminal ID is also the stable native-session ID used for reattachment. The worker writes portable visible-screen checkpoints and sequenced output to SQLite, so quitting Desktop or losing an SSH connection leaves the command running and lets a new viewer recover from a bounded journal. Interactive writer and resize leases prevent multiple viewers from fighting over one PTY. For remote workspaces the helper runs on the SSH host beside the process; Desktop installs the versioned static binary through the user's OpenSSH configuration. Sessions loaded from older manifests or protocol-v1 databases remain attached through their existing worker until explicitly restarted, avoiding deployment-time disruption. Machinen persists workspace, tile, and terminal IDs together with backend, launch definition, working directory, lifecycle state, grouping, and order in `~/Library/Application Support/Machinen/terminals.json`; native recovery data lives separately in `sessions.sqlite3`.

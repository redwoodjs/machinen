...
Machinen's product boundary is terminals plus automation. A versioned, same-user Unix-socket API now creates and arranges workspace tiles, launches commands, labels tiles, sends PTY input, streams lifecycle/output events, and controls the camera. The JSON state file remains private persistence rather than an API. See [`API.md`](API.md) for the complete protocol. The scripting language remains intentionally undecided.

...
open class TerminalView: NSView, NSTextInputClient, NSUserInterfaceValidations, TerminalDelegate {
    ...
    // Visible, unchanged rows reuse their attributed strings and CoreText
    // layout. Entries are pruned to the current viewport on each draw.
    var lineRenderCache: [ObjectIdentifier: CachedLineRendering] = [:]

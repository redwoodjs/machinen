...
/// Cached display data for an unchanged BufferLine. BufferLine's generation
/// advances on every content mutation, allowing scrolling rows to retain their
/// expensive CoreText shaping while their drawing origin changes.
struct CachedLineRendering {
...
extension TerminalView {
    ...
    func resetCaches ()
    {
        ...
        self.trueColors = [:]
        #if os(macOS)
        self.lineRenderCache.removeAll(keepingCapacity: true)
        #endif
    }
    ...
    func buildAttributedString (row: Int, line: BufferLine, cols: Int) -> ViewLineInfo
    {
        ...
        pendingText.reserveCapacity(cols)
        var pendingAttrs: [NSAttributedString.Key: Any]? = nil
        var lastAttr: Attribute? = nil
        var lastHasUrl = false
        var lastIsSelected = false
        // Terminal output normally uses one attribute for most or all cells in
        // a row. Avoid hashing that same Attribute into `attributes` once per
        // cell; resolve it again only at an attribute/link-state boundary.
        var resolvedAttribute: Attribute?
        var resolvedHasUrl = false
        ...
        while col < cols {
            ...
            let attributes: [NSAttributedString.Key: Any]?
            if resolvedAttribute == attr, resolvedHasUrl == hasUrl {
                attributes = resolvedAttributes
            } else {
                let resolved = getAttributes(attr, withUrl: hasUrl)
                resolvedAttribute = attr
                resolvedHasUrl = hasUrl
    ...
    #if os(macOS)
    private func prepareLineSegments(_ lineInfo: ViewLineInfo) -> [PreparedViewLineSegment] {
        lineInfo.segments.compactMap { segment in
            ...
            guard let coreTextRuns = CTLineGetGlyphRuns(ctLine) as? [CTRun] else { return nil }
            let runs = coreTextRuns.map { run in
                PreparedTextRun(
                    run: run,
                    attributes: CTRunGetAttributes(run) as? [NSAttributedString.Key: Any] ?? [:]
                )
            }
    ...
    /// Builds a row only when its buffer generation changes. Selection and
    /// pointer-driven link decorations are intentionally left uncached because
    /// they can alter a row without mutating its BufferLine.
    private func cachedLineRendering(row: Int, line: BufferLine, cols: Int) -> CachedLineRendering {
        let hasDynamicDecorations = selection.active || commandActive || linkHighlightRange != nil
        guard !hasDynamicDecorations else {
            let lineInfo = buildAttributedString(row: row, line: line, cols: cols)
            return CachedLineRendering(
                generation: line.generation,
                columns: cols,
                lineInfo: lineInfo,
                preparedSegments: prepareLineSegments(lineInfo)
            )
        }

        let identifier = ObjectIdentifier(line)
        if let cached = lineRenderCache[identifier],
        ...
        let lineInfo = buildAttributedString(row: row, line: line, cols: cols)
        let rendered = CachedLineRendering(
            generation: line.generation,
            columns: cols,
            lineInfo: lineInfo,
            preparedSegments: prepareLineSegments(lineInfo)
        )
        lineRenderCache[identifier] = rendered
        return rendered
    }
    #endif
    ...
    func drawTerminalContents (dirtyRect: TTRect, context: CGContext, bufferOffset: Int)
    {
        ...
        #if os(macOS)
        // Retain only rows that can be drawn in this viewport. Keeping cache
        // entries tied to visible BufferLine identities prevents scrollback
        // history from growing the renderer cache without bound.
        let visibleStart = max(0, min(firstRow, displayBuffer.lines.count))
        let visibleEnd = max(visibleStart, min(lastRow + 1, displayBuffer.lines.count))
        let visibleLineIDs = Set((visibleStart..<visibleEnd).map { ObjectIdentifier(displayBuffer.lines[$0]) })
        lineRenderCache = lineRenderCache.filter { visibleLineIDs.contains($0.key) }
        ...
        for row in firstRow...lastRow {
            ...
            #if os(macOS)
            let renderedLine = cachedLineRendering(row: row, line: line, cols: displayBuffer.cols)
            let lineInfo = renderedLine.lineInfo
            #else
            ...
            // Prepared CoreText layout is cached with unchanged macOS rows.
            #if os(macOS)
            let preparedSegments = renderedLine.preparedSegments
            #else
            ...
            for prepared in preparedSegments {
                ...
                for run in prepared.runs {
                    let coreTextRun = run.run
                    let runGlyphsCount = CTRunGetGlyphCount(coreTextRun)
                    if runGlyphsCount == 0 {
                        continue
                    }
            ...
            for prepared in preparedSegments {
                ...
                for run in prepared.runs {
                    ...
                    let runGlyphs = [CGGlyph](unsafeUninitializedCapacity: runGlyphsCount) { (bufferPointer, count) in
                        CTRunGetGlyphs(coreTextRun, CFRange(), bufferPointer.baseAddress!)
                        count = runGlyphsCount
                    }

                    var coreTextPositions = [CGPoint](repeating: .zero, count: runGlyphsCount)
                    CTRunGetPositions(coreTextRun, CFRange(), &coreTextPositions)
